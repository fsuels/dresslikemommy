...
test('actual French cm/po source selects preserved pouces values in the compact imperial panel', () => {
  const french = JSON.parse(fs.readFileSync(path.join(__dirname, '../preview-before-french.json'), 'utf8'));
  const records = french.state.tables.filter(table => table.id === 'size-chart' || table.id === 'size-chart-cardigan');
  const tables = records.map(record => tableDom(record, 'Guide des tailles - ' + (record.id === 'size-chart-cardigan' ? 'Cardigan' : 'Robe')));
  const env = environment('fr', tables), {ctx} = env;
  const before = JSON.stringify(records);
  const cardigan = records.find(table => table.id === 'size-chart-cardigan');
  const row = cardigan.rows.find(row => row[0] === 'Maman S');
  assert.equal(row[4], '80 cm / 31,5 pouces');
  assert.equal(row[5], '56 cm / 22 pouces');
  assert.equal(row[9], '40 cm / 15,7 pouces');
  const inst = env.instance('mother', 'Cardigan', 'S');
  ctx.unitSystem = 'imperial';
  const html = ctx.renderCard(inst);
  assert.ok(html.includes('data-unit-set="imperial" aria-pressed="true"'));
  for (const [label, value] of [['Poitrine/Buste', '31,5 pouces'], ['Longueur des manches', '22 pouces'], ['Longueur du vêtement', '15,7 pouces']]) {
    assert.ok(html.includes('<dt>' + label + ' (in)</dt><dd>' + value + '</dd>'), label + ' must use its exact French imperial source value');
    assert.ok(!html.includes('<dt>' + label + ' (cm)</dt>'));
  }
  assert.ok(html.includes('<dt>Poids (lbs)</dt><dd>82,7-99,2 lbs</dd>'));
  assert.equal(ctx.normalizeGuideUnit('po'), 'in');
  assert.equal(ctx.normalizeGuideUnit('pouces'), 'in');
  ctx.unitSystem = 'metric';
  const metric = ctx.renderCard(inst);
  assert.ok(metric.includes('<dt>Poitrine/Buste (cm)</dt><dd>80 cm</dd>'));
