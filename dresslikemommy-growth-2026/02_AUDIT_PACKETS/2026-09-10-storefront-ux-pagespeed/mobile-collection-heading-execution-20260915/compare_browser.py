...
before=json.loads((B/'before-cases.json').read_text())+json.loads((B/'before-shared-cases.json').read_text())
after=json.loads((B/'after-cases.json').read_text())+json.loads((B/'after-shared-cases.json').read_text())
old={x['label'].replace('before-',''):x for x in before}; checks=[]
def check(name,ok,detail=None):checks.append({'name':name,'pass':bool(ok),**({'detail':detail} if detail is not None else {})})
for x in after:
 key=x['label'].replace('after-','');o=old.get(key);r=x['h1']['rect'];h=x['hero'];mobile=x['width']<=767
