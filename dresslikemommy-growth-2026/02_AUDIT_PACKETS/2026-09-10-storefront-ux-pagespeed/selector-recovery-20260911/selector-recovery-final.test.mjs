...
const old=read('../candidate/'+asset), next=read('./missing-guide-init-repair/proposed/'+asset);
...
function mobileScript(selectedVariantId=null){const section=read('./native-deeplink-repair/proposed/sections/main-product.liquid');const start=section.indexOf('    (function () {\n      var sticky =');assert(start>0);let script=section.slice(start,section.indexOf('</script>',start));const locale=parse('../candidate/locales/en.default.json');script=script.replaceAll('{{ product.selected_variant.id | json }}',JSON.stringify(selectedVariantId)).replaceAll('{{ section.id }}','test').replace(/\{\{\s*'([^']+)'\s*\|\s*t\s*\|\s*json\s*\}\}/g,(_,key)=>{const value=key.split('.').reduce((a,k)=>a?.[k],locale);assert.equal(typeof value,'string',key);return JSON.stringify(value);});assert(!script.includes('{{'));return script;}

for(const width of [390,1280])test(`native fallback restores controls and both sticky routes at ${width}px`,async()=>{for(const id of exceptions){const data=fixtures.get(id),h=harness(next,body(data),width);try{
 ...
 w.DLM_PRODUCT_PAGE_COPY={en:{selected_size_label:'Size: '}};w.DLMSizeLabelFormatter={formatSizeLabel:()=> 'unsupported-age'};
 ...
 const selectedRaw=doc.querySelector('variant-selects select[name="options[Size]"]').value;assert(doc.querySelector('[data-sticky-mobile-atc-size]').textContent.includes(selectedRaw));assert(!doc.querySelector('[data-sticky-mobile-atc-size]').textContent.includes('unsupported-age'));
...
for (const linked of [false,true]) test(`native fallback honors server-resolved deep link only: ${linked}`,async()=>{for(const id of exceptions){const data=fixtures.get(id),h=harness(next,body(data),390);try{
 const w=h.w,doc=w.document,wrapper=doc.querySelector('[data-product-desktop-ux]'),native=doc.getElementById('ProductSubmitButton-test'),sticky=doc.querySelector('[data-sticky-mobile-atc-button]');let nativeClicks=0;native.addEventListener('click',()=>nativeClicks++);
 w.DLM_PRODUCT_PAGE_COPY={en:{selected_size_label:'Size: '}};
 runInContext(mobileScript(linked?data.variants[0].id:null),h.dom.getInternalVMContext(),{filename:'inline-mobile-deeplink.js'});
 w.initMatchingSetBuilder(wrapper,'test',data);
 await new Promise(resolve=>w.requestAnimationFrame(resolve));
 if(linked){assert.equal(sticky.textContent,'Add to cart');assert(doc.querySelector('[data-sticky-mobile-atc-size]').textContent.includes(data.variants[0].option1));sticky.click();assert.equal(nativeClicks,1);native.disabled=true;await new Promise(resolve=>w.requestAnimationFrame(resolve));assert.equal(sticky.disabled,true);sticky.click();assert.equal(nativeClicks,1);}
 else {assert.match(sticky.textContent,/select size/i);sticky.click();assert.equal(nativeClicks,0);}
}finally{h.close();}}});

test('missing initial size chart no longer prevents Trail Plaid builder initialization',async()=>{
 const data=fixtures.get('7536337125473');
 const markup='<section id="MainProduct-test">'+body(data).replace('<summary>Size chart</summary>','<summary>Size chart</summary><div data-matching-size-guide-content></div>').replace('<div data-product-desktop-ux','<div data-matching-size-guide-snapshot></div><div data-product-desktop-ux')+'</section>';
 const before=harness(read('./proposed/'+asset),markup);try{assert.throws(()=>before.w.initMatchingSizeGuide(before.w.document.querySelector('[data-product-desktop-ux]'),'test',data),/rows/);}finally{before.close();}
 const after=harness(next,markup);try{const doc=after.w.document,wrapper=doc.querySelector('[data-product-desktop-ux]');
 assert.doesNotThrow(()=>after.w.initMatchingSizeGuide(wrapper,'test',data));
 after.w.initMatchingSetBuilder(wrapper,'test',data);
 assert.equal(doc.querySelector('[data-matching-set-builder]').hidden,false);
 const roles=doc.querySelector('[data-matching-set-roles]').textContent;assert.match(roles,/Girl/);assert.match(roles,/Boy/);assert.match(roles,/Mother|Mom/);assert.match(roles,/Father|Dad/);
 assert.equal(doc.querySelectorAll('table').length,0,'Does not invent measurements');
