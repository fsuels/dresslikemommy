document.addEventListener("DOMContentLoaded", function () {
  ...
  const applyFormattedSizeLabels = (root) => {
    ...
    elements.forEach((element) => {
      ...
      // Cart and order lines must retain the exact selected size label.
      // A height label is not a supported age recommendation.
      const displayLabel = String(rawValue || "").trim();
  ...
  const applySizeChartDisplayLabels = (root) => {
    ...
    tables.forEach((tableElement) => {
      ...
      Array.from(tableElement.querySelectorAll("tr")).slice(1).forEach((row) => {
        ...
        if (tableElement.closest('[data-matching-set-fallback="true"]')) {
          if (sizeCell.textContent !== rawValue) sizeCell.textContent = rawValue;
          return;
        }
  ...
  const applyFrontendSizeOptionLabels = (selectElement) => {
    ...
    Array.from(selectElement.options).forEach((optionElement) => {
      ...
      const displayLabel = String(optionElement.value || "").trim();
      if (!displayLabel) return;
