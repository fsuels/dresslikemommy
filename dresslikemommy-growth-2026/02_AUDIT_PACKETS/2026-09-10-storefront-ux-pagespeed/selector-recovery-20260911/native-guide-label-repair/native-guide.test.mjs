...
test('native fallback charts retain raw sizes on initial and dynamic rendering without changing measurements or other guides',async()=>{
  const table='<table class="size-chart"><tr><th>Size</th><th>Length (cm/in)</th><th>Bust (cm/in)</th></tr><tr><td>100cm</td><td>69 / 27.2</td><td>62 / 24.4</td></tr><tr><td>S (Adult Normal Version)</td><td>115 / 45.3</td><td>86 / 33.9</td></tr></table>';
  const markup='<section data-matching-set-fallback="true">'+table+'</section><aside>'+table+'</aside>';
  const a=await boot(before,markup),h=await boot(after,markup);try{
    assert.equal(a.w.document.querySelector('section td').textContent,'2–3Y');
    assert.equal(h.w.document.querySelector('section td').textContent,'100cm');
    assert.equal(h.w.document.querySelector('aside td').textContent,'2–3Y');
    assert.deepEqual(Array.from(h.w.document.querySelectorAll('section td')).map(e=>e.textContent),['100cm','69 / 27.2','62 / 24.4','S (Adult Normal Version)','115 / 45.3','86 / 33.9']);
    h.w.document.querySelector('section').innerHTML=table.replace('<td>100cm</td>','<td data-size-display-raw="100cm">2–3Y</td>');
    await new Promise(resolve=>setImmediate(resolve));assert.equal(h.w.document.querySelector('section td').textContent,'100cm');
    h.w.document.dispatchEvent(new h.w.Event('cart:updated'));assert.equal(h.w.document.querySelector('section td').textContent,'100cm');
  }finally{a.close();h.close();}
});
