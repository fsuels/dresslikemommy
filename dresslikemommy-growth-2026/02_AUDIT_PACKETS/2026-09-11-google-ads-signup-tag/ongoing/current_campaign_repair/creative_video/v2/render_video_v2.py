...
VIDEO = HERE / "mommy_and_me_15s_silent_draft_v2.mp4"
...
SAMPLE_TIMES = [1.0, 4.966667, 5.0, 7.5, 9.966667, 10.0, 12.5, 14.0, 14.966667]
...
title_font = ImageFont.truetype(FONT_REGULAR, 90)
cta_font = ImageFont.truetype(FONT_BOLD, 84)
base = Image.new("RGB", (WIDTH, HEIGHT), BACKGROUND)
brand_box = centered_text(base, BRAND, 24, brand_font)
...
for index, scene in enumerate(SCENES):
    ...
    scale = min(1664 / source.width, 704 / source.height)
    size = (round(source.width * scale), round(source.height * scale))
    photo = source.resize(size, Image.Resampling.LANCZOS)
    left, top = (WIDTH - photo.width) // 2, 232
    photo_box = [left, top, left + photo.width, top + photo.height]
    frame = base.copy()
    frame.paste(photo, (left, top))
    title_box = centered_text(frame, scene["title"], 125, title_font)
    cta_box = centered_text(frame, scene["cta"], 968, cta_font)
    ...
    try:
        for scene_index, frame in enumerate(frames):
            ...
            for local_frame in range(5 * FPS):
                # Direct cuts at frames150/300: every frame has an intact photograph.
                process.stdin.write(still_bytes)
        process.stdin.close()
...
mobile_dir = HERE / "mobile_qa"
mobile_dir.mkdir()
for sample in samples:
    path = mobile_dir / Path(sample["file"]).name
    with Image.open(HERE / sample["file"]) as image:
        image.resize((360, 203), Image.Resampling.LANCZOS).save(path)
    sample["mobile_qa_file"] = "mobile_qa/" + path.name
    sample["mobile_qa_dimensions"] = [360, 203]
    sample["mobile_qa_sha256"] = digest(path)
...
photo_checks = []
for sample in samples:
    scene_index = min(int(sample["requested_time_seconds"] // 5), 2)
    with Image.open(HERE / sample["file"]) as sample_image:
        decoded_photo = sample_image.convert("RGB").crop(sources[scene_index]["video_photo_rectangle"])
    from PIL import ImageChops
    assert max(ImageStat.Stat(decoded_photo).stddev) > 10, "Blank or nearly blank photo region"
...
v1_preservation = json.loads((HERE / "V1_PRESERVATION.json").read_text())
assert all(digest(HERE.parent / p) == h for p, h in v1_preservation["before_sha256"].items())
v1_preservation["status"] = "VERIFIED_UNCHANGED_AFTER_V2_RENDER"
save_json("V1_PRESERVATION.json", v1_preservation)
...
save_json("manifest.json", {
    ...
    "revision": "v2", "revision_reason": "Larger mobile-readable text and direct scene cuts remove v1's small captions and blank transition gaps.",
    "acceptance_source": "../../CREATIVE_ACCEPTANCE.json", "acceptance_sha256": acceptance_hash,
    "v1_preserved": "V1_PRESERVATION.json",
    ...
    "transitions": "Direct cuts at5.000s and10.000s. No fades, all-white gaps, overlapping photo composites or garment resynthesis. Final CTA holds through end.",
    "typography": {"brand_font_px": 90, "headline_font_px": 90, "cta_font_px": 84,
                   "visible_text_heights_px": [[b[3] - b[1] for b in src["text_rectangles"]] for src in sources],
                   "equivalent_visible_text_heights_at_360px_width": [[round((b[3] - b[1]) *360/1920, 2) for b in src["text_rectangles"]] for src in sources],
                   "full_photo_rectangle_y": [232, 936], "text_photo_overlap": False},
...
save_json("VALIDATION.json", {"status": "TECHNICAL_PASS_VISUAL_REVIEW_PENDING", "duration_seconds": 15,
                             ...
                             "no_blank_transition_sample_frames": "PASS_AT_4.967_5.000_9.967_10.000_SECONDS",
                             "v1_all_artifact_hashes_unchanged": True, "mobile_qa_frames_360px_wide": len(samples),
