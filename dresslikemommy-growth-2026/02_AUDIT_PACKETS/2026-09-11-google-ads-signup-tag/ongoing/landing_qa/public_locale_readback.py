...
def fetch(locale, url, kind):
    ...
    for attempt in range(2):
        if STOP_REQUESTS.is_set():
            row.update({"extraction_status": "SKIPPED_GLOBAL_STOP", "error": "Earlier request hit rate/auth/challenge stop; no network call made"})
            break
        row["attempts"] += 1
        try:
...
def main():
    ...
    for locale in ["ro", "nl", "en"]:
        if STOP_REQUESTS.is_set():
            deep.append({"locale": locale, "extraction_status": "SKIPPED_GLOBAL_STOP"})
            continue
