#!/usr/bin/env python3
"""ES/IT Golden Daisy isolated-browser checkout-to-shipping QA.
...
VARIANT_ID = "44197959499873"
LANE_DIR = Path(__file__).resolve().parent
RAW_DIR = LANE_DIR / "raw"
SCREENSHOT_DIR = LANE_DIR / "screenshots"
SUMMARY_PATH = LANE_DIR / "es_it_golden_daisy_checkout_to_shipping_summary.json"
REPORT_PATH = LANE_DIR / "ES_IT_GOLDEN_DAISY_CHECKOUT_TO_SHIPPING.md"
    ...
    expression = r"""
        (async () => {
          ...
          const fill = (tokens, value) => {
            ...
            const wantsPhone = tokenList.some(token => token === 'phone' || token === 'tel');
            const el = all().find(input => {
              const label = labelFor(input);
              if (input.type === 'hidden') return false;
              if (wantsPhone && (input.type === 'email' || input.name === 'email' || input.id === 'email')) return false;
              return tokenList.some(token => label.includes(token));
            });
...
def build_summary(client: CdpClient, country_code: str) -> dict[str, Any]:
    ...
    product_url = f"{BASE_URL}/{market['locale_path']}/products/{HANDLE}?variant={VARIANT_ID}&country={country_code}"
    ...
    return {
        ...
        "lane": "ES/IT Golden Daisy isolated-browser checkout-to-shipping QA",
        "country_code": country_code,
        "problem_id": "PROB-2026-05-09-NON-US-SEARCH-NATIVE-LANGUAGE-COPY-GATE",
...
def main() -> None:
    ...
    parser.add_argument("--countries", nargs="+", choices=sorted(MARKETS), default=["ES", "IT"])
