#!/usr/bin/env python3
"""AU isolated-browser checkout-to-shipping QA.
...
VARIANT_ID = "41871520661601"
PRODUCT_URL = f"{BASE_URL}/products/{HANDLE}?variant={VARIANT_ID}&country=AU"
LANE_DIR = Path(__file__).resolve().parent
RAW_DIR = LANE_DIR / "raw"
SCREENSHOT_DIR = LANE_DIR / "screenshots"
PROFILE_DIR = RAW_DIR / "chrome-au-isolated-profile"
SUMMARY_PATH = LANE_DIR / "au_isolated_checkout_to_shipping_summary.json"
REPORT_PATH = LANE_DIR / "AU_ISOLATED_CHECKOUT_TO_SHIPPING.md"
...
def cart_probe(client: CdpClient) -> dict[str, Any]:
...
def fill_checkout(client: CdpClient) -> dict[str, Any]:
    return runtime_eval(
        client,
        r"""
        (async () => {
          ...
          const data = {
            email: 'dlm-au-readonly-' + Date.now() + '@example.com',
            firstName: 'Readonly',
            lastName: 'QA',
          ...
          selectByText(['province', 'state', 'zone'], data.province);
          await sleep(3000);
...
def checkout_state(client: CdpClient) -> dict[str, Any]:
    return runtime_eval(
        client,
        r"""
        (() => {
          const text = document.body ? document.body.innerText : '';
          const rates = text.split(/\n+/).filter(line => /(standard|express|delivery|shipping|aud|\$|free|gratis)/i.test(line)).slice(0, 80);
...
def launch_chrome(port: int) -> subprocess.Popen[str]:
...
def build_summary(client: CdpClient) -> dict[str, Any]:
    ...
    product["screenshot"] = save_screenshot(client, "au-product-aud.png")
    product["url"] = redacted_url(product.get("url", ""))

    cart_api = cart_probe(client)
    network_events.append({"step": "cart_api_probe", "data": cart_api})
    navigate(client, f"{BASE_URL}/cart")
    cart = snapshot(client)
    cart["screenshot"] = save_screenshot(client, "au-cart-before-checkout.png")
    ...
    checkout_entry["screenshot"] = save_screenshot(client, "au-checkout-entry.png")
    checkout_entry["url"] = redacted_url(checkout_entry.get("url", ""))

    fill_result = fill_checkout(client)
    time.sleep(8)
    state = checkout_state(client)
    if not isinstance(state, dict) or state.get("runtime_exception"):
        ...
        fallback_lines = [
            ...
            if re.search(r"(standard|express|delivery|shipping|aud|\$|free|gratis)", line, re.I)
        ][:80]
        state = {
            ...
            "has_aud": bool(re.search(r"\\bAUD\\b|\\$\\s*\\d", fallback.get("text", ""))),
    ...
    state["screenshot"] = save_screenshot(client, "au-checkout-shipping-rates.png")
    ...
    shipping_ui_pass = bool(state.get("has_standard") and state.get("has_express") and state.get("has_aud") and not state.get("has_order_confirmation"))
    api_rates_pass = bool(rates)
    decision = (
        "AU_CHECKOUT_TO_SHIPPING_PASSED_READONLY_NO_PAYMENT_NO_ORDER"
        if shipping_ui_pass or api_rates_pass
        else "AU_CHECKOUT_STILL_BLOCKED_OR_RATES_NOT_VISIBLE"
    )
    return {
        "generated_at": datetime.now().astimezone().isoformat(timespec="seconds"),
        "lane": "AU isolated-browser checkout-to-shipping QA",
        "problem_id": "PROB-2026-05-08-AU-CHECKOUT-429",
...
def write_report(summary: dict[str, Any]) -> None:
    ...
    lines.extend(
        [
            ...
            f"- UI contains AUD / money signal: `{state.get('has_aud')}`",
