"""Validate this exact German translation operation and its independent readback."""
import hashlib
import json
...
PRIVATE = Path('/Users/fsuels/.config/dresslikemommy/merchant-evidence/20260914-de-translation-repair')
...
assert proposal['locale'] == 'de' and proposal.get('marketId') is None
assert nodes(before) == nodes(fresh), 'Source or German translations changed before write'
...
for i in range(10):
    ...
    for field in variables['translations' + str(i)]:
        ...
        assert field == {'locale': 'de', 'key': expected['key'], 'value': expected['value'], 'translatableContentDigest': source['digest']}
    ...
    for rid, original in resources.items():
        ...
        for key, old in old_fields.items():
            ...
            if (rid, key) in changes:
                assert new == {'key': key, 'value': changes[(rid, key)]['value'], 'locale': 'de', 'outdated': False}
            else:
                assert old == new, 'Untargeted German field drift'
                unchanged += 1
    assert unchanged == 25 and source_fields == 60
    result.update(status='VERIFIED_25_EXACT_GERMAN_CHANGES', privateAfterSha256=sha(PRIVATE / 'after.json'),
                  afterCompletedAt=after['completedAt'], unchangedGermanFields=unchanged,
