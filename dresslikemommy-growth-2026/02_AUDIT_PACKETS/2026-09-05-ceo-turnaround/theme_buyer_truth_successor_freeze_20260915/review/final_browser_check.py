...
def inspect_guide(name, locale, role, garment, unit, width):
    ...
    check(name + ":one_matching_selected_row", len(selected) == 1 and (row[0] == "S" if role == "mother" else row[0] in ["2 Years", "2 ans", "سنتين"]))
    ...
    check(name + ":actual_role_garment_and_size_context", len(matching_triggers) == 1 and bool(row) and matching_triggers[0]["data-fit-size-label"] == row[0])
...
report = {
    ...
    "limits": ["Independent local evaluation of root-owned raw captures, no reviewer browser actions.", "Numeric chest and garment-length values are compared with source cells; localized suffixes and extra valid height columns are retained, not treated as formatting failures.", "Unit controls and role-switch resets retain their pre-existing separate behavior; no cross-role persistence claim.", "Only cited accepted captures plus CTA/AU evidence support this review; diagnostic/intermediate files are excluded from acceptance."],
}
destination = P / "review/final-browser-independent-checks.json"
