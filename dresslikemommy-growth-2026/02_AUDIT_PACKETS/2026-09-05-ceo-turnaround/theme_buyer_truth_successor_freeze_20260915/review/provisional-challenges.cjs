...
console.log('PROVISIONAL_SOURCE_SHA256', sourceSha256);
const observations = [];
{
  ...
  const result = ctx.findMeasurementsForOption(group, option, ctx.getMeasurementContextForInstance(group, inst, option));
  observations.push({case:'available Mother M option but source chart contains only Mother L', expected:'no tooltip measurements for a different size', actualRow:result && result.row[0], sourceTable:result && result.sourceTable && result.sourceTable.id, pass:result === null});
}
{
  const env = environment(), {ctx} = env;
  const record = JSON.parse(JSON.stringify(env.tables[1].record));
  record.headers = record.headers.map(h => h.includes('Sleeve Length') ? 'Dress Length (cm/in)' : h);
  const contradictory = tableDom(record, 'Size Chart - Cardigan');
  env.setTables([env.tables[0], contradictory]);
  const result = ctx.getFitGroupFromProductTables('mother','mother','cardigan');
  observations.push({case:'Cardigan table ID/heading but explicit Dress Length column', expected:'do not present another garment column as Cardigan measurements', actualHeaders:result && result.headers, pass:result === null || !JSON.stringify(result.headers).includes('Dress Length')});
}
console.log(JSON.stringify({sourceSha256, observations},null,2));
