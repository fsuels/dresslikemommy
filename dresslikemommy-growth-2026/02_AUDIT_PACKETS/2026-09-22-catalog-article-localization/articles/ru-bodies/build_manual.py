...
def parts(s):
    ...
    return out
rows=json.loads((BASE/'baseline.json').read_text())['rows']
segments={};data=[]
for r in rows:
...
claims={x['resourceId']:x['claims'] for x in json.loads((BASE.parent/'source_claims_release_review.json').read_text())['articles']}
...
for r in rows:
    ...
    if f.exists():value=f.read_text().rstrip('\n');missing=[]
        ...
        for tag,t in parts(r['sourceValue']):
            ...
            out.append(prefix+translations[id]+t[len(t.rstrip()):])
        value=''.join(out)
    ...
    checks.append({'resourceId':r['resourceId'],**offline.verify_text(r['sourceValue'],value,'ru')})
