...
for name,obj in [('mapping_dictionary_v2.json',result),('semantic_coverage1926_v2.json',{'rows':coverage}),('context_retained_occurrences_v2.json',{'rows':exceptions})]:(H/name).write_text(json.dumps(obj,ensure_ascii=False,indent=2)+'\n')
(H/'mapping_dictionary_v2.tsv').write_text('map\tgroup\tlocale\tsource\tbefore\tafter\tcurrent\toutdated\n'+''.join(f"{x['mappingIndex']}\t{x['semanticIndex']}\t{x['locale']}\t{x['sourceHeader']}\t{x['beforeHeader']}\t{x['afterHeader']}\t{x['currentCount']}\t{x['outdatedCount']}\n" for x in rows))
print(result['counts']);print(hashlib.sha256((H/'mapping_dictionary_v2.json').read_bytes()).hexdigest())
