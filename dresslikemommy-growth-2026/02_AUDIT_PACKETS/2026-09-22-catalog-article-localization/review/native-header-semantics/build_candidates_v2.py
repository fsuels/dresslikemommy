...
mp=json.loads((H/'mapping_dictionary_v2.json').read_text());by=collections.defaultdict(list);held=[];superseded=[]
for m in mp['rows']:
 for u in m['uses']:
...
for name,obj in [('native_header_candidates_v2.json',{'summary':summary,'rows':rows}),('candidate_checks_v2.json',{'summary':summary,'checks':checks,'rawFileBindings':rawchecks,'overlayBeforeMismatches':mismatches}),('native_header_cell_ledger_v2.json',{'rows':cellledger}),('outdated_hold_ledger_v2.json',{'rows':held}),('superseded_occurrences_v2.json',{'rows':superseded}),('application_failures_v2.json',{'rows':failures})]:(H/name).write_text(json.dumps(obj,ensure_ascii=False,indent=2)+'\n')
print(json.dumps(summary,ensure_ascii=False));print('candidate SHA',hashlib.sha256((H/'native_header_candidates_v2.json').read_bytes()).hexdigest())
