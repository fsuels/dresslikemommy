...
I=P/'review/body-structure-audit/effective_body_inventory.json';S=P/'review/body-structure-audit/tag_candidates_v4.json';M=O/'proposed_language_mappings_v3.json';N=O/'native_mapping_supplement.json';B=O/'native_header_bound_occurrences.json'
assert fs(S)=='a0a54bd0bcdce59936aad065f94e069950ee2bb820fd5a1e2666536bdc8f1c1f'
...
for r in base:
 ...
 ck={'resourceId':k[0],'locale':k[1],'headerCellsChanged':len(changes),'sourceSHA256Pass':sha(r['source'])==r['sourceSHA256'],'beforeValueSHA256':sha(v),'valueSHA256':sha(value),'rawBeforeValueSHA256':sha(r['rawBefore']['value']) if r['rawBefore'] else None,'nonHeaderBytesExact':mask(v)==mask(value),'tagBytesExact':TAGS.findall(v)==TAGS.findall(value),'independentHTMLParserTagAttributeEventsExact':a.events==b.events,'allNumericTokensExact':NUM.findall(v)==NUM.findall(value),'allDataCellsExact':TD.findall(v)==TD.findall(value),'headerCountExact':len(list(HP.finditer(v)))==len(list(HP.finditer(value))),'structuralV4OverlayApplied':bool(sr),'priorVerifiedBodyOverlayApplied':r['overlayApplied']}
...
out=O/'header_candidates_v1.json';out.write_text(json.dumps({'rows':rows},ensure_ascii=False,indent=2)+'\n')
manifest={'rows':len(rows),'appliedHeaderCells':len(applied),'englishAndOrthographyCells':sum(x['mappingLane'].startswith('ROOT_REVIEWED_ENGLISH') for x in applied),'nativeCorruptionCells':sum(x['mappingLane'].startswith('ROOT_REVIEWED_NATIVE') for x in applied),'outdatedBodyTuplesHeld':len({(r['resourceId'],r['locale']) for r in held}),'outdatedHeaderOccurrencesHeld':len(held),'nativeAlignmentOccurrencesHeld':len(J(O/'native_header_alignment_holds.json')['rows']),'structuralV4OverlapBodies':sum(c['structuralV4OverlayApplied'] for c in checks),'priorVerifiedOverlayBodies':sum(c['priorVerifiedBodyOverlayApplied'] for c in checks),'candidateFile':str(out.relative_to(P)),'candidateSHA256':fs(out),'structuralV4File':str(S.relative_to(P)),'structuralV4SHA256':fs(S),'mappingFile':str(M.relative_to(P)),'mappingSHA256':fs(M),'nativeMappingFile':str(N.relative_to(P)),'nativeMappingSHA256':fs(N),'allCandidatePreservationChecksPass':True,'perLocale':dict(collections.Counter(r['locale'] for r in rows)),'externalWrites':'NONE','releaseOrder':'ROOT must verify structural375v4 then fresh exact body-source/global-before guards before header release.'}
for n,d in [('candidate_checks_v1.json',{'rows':checks,'allPass':True}),('applied_header_cell_ledger_v1.json',{'rows':applied}),('outdated_body_holds_v1.json',{'rows':held}),('structural_v4_header_reparse.json',{'rows':overlaydelta}),('candidate_manifest_v1.json',manifest)]: (O/n).write_text(json.dumps(d,ensure_ascii=False,indent=2)+'\n')
