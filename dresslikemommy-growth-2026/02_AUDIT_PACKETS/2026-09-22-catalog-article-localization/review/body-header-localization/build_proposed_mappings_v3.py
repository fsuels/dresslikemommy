...
d=J(O/'mapping_matrix_v2.json');D={l:dict(zip(d['keys'],v)) for l,v in d.items() if l!='keys'}
...
def translate(loc,h):
 ...
 if '|' in h and loc=='ko':return h.split('|',1)[1].replace('가슴 둘레','가슴'),'Retain the complete existing Korean label; remove duplicate English label in the same header.'
 ...
 if stem in base:v=q[base[stem]]
  ...
  v=q['pants']+'/'+(q['shorts'] if loc=='de' else q['shorts'].lower())
...
out=O/'proposed_language_mappings_v3.json';out.write_text(json.dumps({'status':'AUTHOR_PROPOSED_MAPPING_ONLY_PENDING_ROOT_INDEPENDENT_MEANING_REVIEW','rows':output},ensure_ascii=False,indent=2)+'\n')
