...
H=Path(__file__).resolve().parent;B=H.parents[2];sha=lambda s:hashlib.sha256(s.encode()).hexdigest();W=json.loads((H/'worklist40.json').read_text())['rows'];P={x['productIndex']:x['nodes'] for x in json.loads((H/'source_prose_nodes.json').read_text())['rows']};D=runpy.run_path(str(H/'table_dictionary.py'));T={l:runpy.run_path(str(H/f'prose_{l}.py'))['TEXTS'] for l in ['ar','he','hi','ja','ko']}
...
for r in W:
 ...
 value=''.join(translated);assert Audit(source).events==Audit(value).events;assert not re.search('QZXTOKEN|QZX|XTOKEN',value);assert num(' '.join(tablesource))==num(' '.join(tabletarget))
...
for name,obj in [('candidates40.json',{'summary':summary,'rows':rows}),('checks40.json',{'summary':summary,'rows':checks}),('node_pairs40.json',{'rows':pairs})]:(H/name).write_text(json.dumps(obj,ensure_ascii=False,indent=2)+'\n')
print(summary);print('candidateSHA',hashlib.sha256((H/'candidates40.json').read_bytes()).hexdigest())
