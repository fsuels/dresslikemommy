...
inputs=['review/product-remainder-review/held_old15_candidate.json','review/product-remainder-review/held_nochart5_candidate.json']
...
write('product20_checks.json',{'status':'PASS'if not any(x['errors']for x in checks)else'FAILED','rows':checks,'exactUnchangedNumericCells':numeric})
write('product20_pairs.json',pp);write('product20_inline.json',[{'i':i,'locale':loc,'text':s,'tuples':ids}for i,((loc,s),ids)in enumerate(joins.items())]);write('product20_candidates_frozen_copy.json',{'rows':rows})
