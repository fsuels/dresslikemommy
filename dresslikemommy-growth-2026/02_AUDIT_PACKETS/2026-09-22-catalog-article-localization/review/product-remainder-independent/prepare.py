...
inputs=['review/product-remainder-review/retained_body24_candidate.json','review/product-remainder-review/held_ru_sv36_candidate.json']
...
write('product60_checks.json',{'status':'PASS'if not any(x['errors']for x in checks)else'FAILED','rows':checks,'exactUnchangedNumericCells':numeric})
write('product60_pairs.json',pp);write('product60_inline.json',[{'i':i,'locale':loc,'text':s,'tuples':ids}for i,((loc,s),ids)in enumerate(joins.items())]);write('product60_candidates_frozen_copy.json',{'rows':rows})
