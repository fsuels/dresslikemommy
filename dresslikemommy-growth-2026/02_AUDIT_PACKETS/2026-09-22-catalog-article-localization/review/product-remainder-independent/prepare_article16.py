...
inputs=['review/other-article-bodies/candidate.json']
rawcache={};rows=[];checks=[];pairs={};joins={};numeric=0
for name in inputs:
 f=PACK/name;rr=json.loads(f.read_text())['rows']
 for r in rr:
  src=r.get('sourceValue',r.get('source'));raw=PACK/r['rawFile']
  ...
  tokenss=re.split(r'(<[^>]+>)',src);tokenst=re.split(r'(<[^>]+>)',r['value']);assert len(tokenss)==len(tokenst),(ident,'token count')
  st=[(html.unescape(s).strip(),html.unescape(t).strip()) for s,t in zip(tokenss,tokenst) if not s.startswith('<') and (s.strip() or t.strip())];ss=[s for s,t in st];tt=[t for s,t in st]
...
write('article16_checks.json',{'status':'PASS'if not any(x['errors']for x in checks)else'FAILED','rows':checks,'exactUnchangedNumericCells':numeric})
write('article16_pairs.json',pp);write('article16_inline.json',[{'i':i,'locale':loc,'text':s,'tuples':ids}for i,((loc,s),ids)in enumerate(joins.items())]);write('article16_candidates_frozen_copy.json',{'rows':rows})
