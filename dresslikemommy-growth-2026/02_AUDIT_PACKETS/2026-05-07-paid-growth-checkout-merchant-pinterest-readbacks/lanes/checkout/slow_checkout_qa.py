#!/usr/bin/env python3
"""Slow, no-payment storefront checkout QA for ES/IT/RO/PT.
...
def page_summary(country_code: str, kind: str, response: dict[str, Any]) -> dict[str, Any]:
    ...
    shopify_currency = ""
    currency_signal_match = re.search(
        r"Shopify\.currency\s*=\s*\{[^}]*active\s*:\s*[\"']([^\"']+)",
        body,
        flags=re.I,
    )
    if not currency_signal_match:
        currency_signal_match = re.search(
            r"[\"'](?:currency|presentment_currency)[\"']\s*:\s*[\"']([A-Z]{3})[\"']",
            body,
            flags=re.I,
        )
    if currency_signal_match:
        shopify_currency = clean(currency_signal_match.group(1)).upper()
    limit_hits = []
    ...
    return {
        ...
        "shopify_currency_signal": shopify_currency,
...
def markdown_report(data: dict[str, Any]) -> str:
    ...
    for code, rows in data["pages_by_country"].items():
        ...
        for page in rows:
            ...
            lines.append(
                "| {kind} | {http} | {lang} | {currency} | {shopify_currency} | {url} | {finding} |".format(
                    kind=page["kind"],
                    http=page.get("http_status", ""),
                    lang=page.get("html_lang") or "-",
                    currency=page.get("currency_meta") or "-",
                    shopify_currency=page.get("shopify_currency_signal") or "-",
...
def main() -> None:
    ...
    route_failures = [
        page
        for rows in pages_by_country.values()
        for page in rows
        if page.get("http_status") and page.get("http_status") != 200
    ]
    product_currency_mismatches = [
        (code, page.get("currency_meta") or page.get("shopify_currency_signal") or "")
        for code, rows in pages_by_country.items()
        for page in rows
        if page.get("kind") == "product"
        and (
            (page.get("currency_meta") and page.get("currency_meta") != COUNTRIES[code]["currency"])
            or (
                not page.get("currency_meta")
                and page.get("shopify_currency_signal")
                and page.get("shopify_currency_signal") != COUNTRIES[code]["currency"]
            )
        )
    ]
    ...
    route_failure_summary = ", ".join(
        f"{page['country_code']}:{page['kind']} HTTP {page['http_status']}" for page in route_failures
    ) or "none"
    currency_mismatch_summary = ", ".join(
        f"{code}:{currency or 'missing'}" for code, currency in product_currency_mismatches
    ) or "none detected"
    ...
    findings = [
        ...
        f"Route HTTP failures in this run: {route_failure_summary}.",
        f"Product currency meta/signals not matching expected EUR: {currency_mismatch_summary}.",
    ...
    if route_failures:
        route_decision = "route_readback_not_clean_for_all_target_pages"
    else:
        route_decision = "route_readback_passed_for_target_pages"
    if product_currency_mismatches:
        currency_decision = "product_currency_meta_not_clean_for_target_markets"
    else:
        currency_decision = "product_currency_meta_not_blocked_in_probe"
    next_action = (
        "Keep ES/IT/RO/PT out of live paid spend until the parent integrates this with catalog/feed, "
        "tracking, route/currency, and economics gates. If spend is later considered, perform a human "
        "browser checkout walkthrough through the shipping step only; do not submit payment."
    )
