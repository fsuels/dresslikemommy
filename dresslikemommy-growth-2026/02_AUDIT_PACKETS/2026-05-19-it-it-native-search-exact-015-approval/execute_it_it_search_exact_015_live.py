#!/usr/bin/env python3
"""Create/read back the approved Dress Like Mommy IT_IT native Search exact CPC test.
...
CAMPAIGN_NAME = "DLM_IT_SEARCH_NATIVE_IT_EXACT_015_TEST_20260519"
BUDGET_MICROS = 5_000_000
MAX_CPC_BID_MICROS = 150_000
CUSTOMER_ID = "3990976848"
ITALY_GEO_TARGET = "geoTargetConstants/2380"
ITALIAN_LANGUAGE = "languageConstants/1004"

KEYWORDS_CSV = PACKET_DIR / "it_it_search_exact_015_launch_keywords.csv"
NEGATIVES_CSV = PACKET_DIR / "it_it_search_exact_015_negative_keywords.csv"
RSA_CSV = PACKET_DIR / "it_it_search_exact_015_rsa_assets.csv"
...
def validate_inputs(keyword_rows: list[dict[str, str]], negative_rows: list[dict[str, str]], rsa_rows: list[dict[str, str]]) -> dict[str, Any]:
    ...
    for row in keyword_rows:
        ...
        if int(row["max_cpc_bid_micros"]) != MAX_CPC_BID_MICROS:
            issues.append(f"Keyword CPC is not 150000 micros: {row['keyword']}")
        if not row["final_url"].startswith("https://www.dresslikemommy.com/it/"):
            issues.append(f"Unexpected IT localized final URL: {row['final_url']}")
...
def build_operations(
    ...
    for criterion_type, resource_name in [("location", ITALY_GEO_TARGET), ("language", ITALIAN_LANGUAGE)]:
...
def write_report(path: Path, payload: dict[str, Any]) -> None:
    ...
    ads = after.get("ads", [])
    report = [
        "# Google Ads API IT_IT Native Search Validate/Execution Report",
        "",
...
def main() -> int:
    ...
    timestamp = now_stamp()
    approval_phrase = (
        "Approve enabling the Google Ads Search campaign DLM_IT_SEARCH_NATIVE_IT_EXACT_015_TEST_20260519 exactly as specified in "
        "IT_IT_SEARCH_EXACT_015_APPROVAL_PACKET.md: Google Search only, Italy, Italian, exact match only, "
        "$5/day max budget, $0.15 max CPC, purchase-only conversion, no broad match, no phrase match, no AI Max, no PMax, no Search Partners, "
    ...
    before_file = PACKET_DIR / f"google_ads_api_it_it_before_readback_{timestamp}.json"
    write_json(
        before_file,
        {
    ...
    if before["campaigns"] and not args.allow_existing_readback:
        ...
        report_file = PACKET_DIR / f"google_ads_api_it_it_execution_report_{timestamp}.md"
    ...
    after_file = PACKET_DIR / f"google_ads_api_it_it_after_readback_{timestamp}.json"
    write_json(
        after_file,
        {
            "timestamp_utc": timestamp,
