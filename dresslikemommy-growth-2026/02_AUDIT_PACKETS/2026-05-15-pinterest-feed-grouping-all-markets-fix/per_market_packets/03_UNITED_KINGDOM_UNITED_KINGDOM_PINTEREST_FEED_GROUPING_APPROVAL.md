# Pinterest Feed Grouping Approval Packet — United Kingdom

Shopify Market handle: `united-kingdom`
Owner priority rank: 3
Languages: en
...
Capture for `united-kingdom` only:

1. Pinterest Ads Manager -> Catalogs -> advertiser `549756244483` -> catalog item count for the `united-kingdom` source (or the merged catalog if Pinterest does not split by Shopify Market).
2. Pinterest "Dresses" group entry count where source/market = `united-kingdom` (if Pinterest exposes per-source filters; otherwise capture overall).
3. Sample item-ID format — expected pattern: `shopify_UNITED_KINGDOM_<parent>_<variant>`.
...
6. Confirm the connected advertiser ID matches `549756244483` and the Shopify Market mapping is `united-kingdom`.
...
2. If the channel supports per-market overrides, select the `united-kingdom` Shopify Market context. Otherwise the global channel setting applies to every market and a single toggle covers all of them — record which behavior the UI exposes.
...
If the toggle does not exist for this market, STOP. Fall back to Path B (custom GraphQL feed generator) for `united-kingdom` only — see `../scripts/generate_pinterest_feed_grouped.py` and upload its `united-kingdom` output as a separate Pinterest catalog source.
...
1. New Pinterest catalog item count for the `united-kingdom` source.
...
7. Run `python3.13 ops/scripts/check_pinterest_feed_grouping.py --market united-kingdom` -> expect `PASS`.

## Rollback

If after-state readback shows fewer working products than expected, missing hero images, or new disapprovals:

1. Return to the same Shopify Pinterest channel settings page (in the `united-kingdom` context if per-market).
...
## Why This Matters for `United Kingdom`

- Currency is `GBP`, so wasted impressions on duplicate variant items burn `GBP` budget directly.
...
## Acceptance Criteria for `united-kingdom`

- Catalog rows for `united-kingdom` collapse from ~20x variant inflation to one row per parent OR rows sharing `item_group_id`.
- `image_link` on every row is the parent product's featured image.
- `additional_image_link` carries variant images.
- Guardrail `check_pinterest_feed_grouping.py --market united-kingdom` returns PASS.
- Pinterest catalog shows no new severity-1 disapprovals for `united-kingdom`.
