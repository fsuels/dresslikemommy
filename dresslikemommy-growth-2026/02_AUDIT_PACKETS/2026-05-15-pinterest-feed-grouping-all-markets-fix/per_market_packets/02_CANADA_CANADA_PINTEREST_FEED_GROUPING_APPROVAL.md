# Pinterest Feed Grouping Approval Packet — Canada

Shopify Market handle: `canada`
Owner priority rank: 2
Languages: en, fr
...
Capture for `canada` only:

1. Pinterest Ads Manager -> Catalogs -> advertiser `549756244483` -> catalog item count for the `canada` source (or the merged catalog if Pinterest does not split by Shopify Market).
2. Pinterest "Dresses" group entry count where source/market = `canada` (if Pinterest exposes per-source filters; otherwise capture overall).
3. Sample item-ID format — expected pattern: `shopify_CANADA_<parent>_<variant>`.
...
6. Confirm the connected advertiser ID matches `549756244483` and the Shopify Market mapping is `canada`.
...
2. If the channel supports per-market overrides, select the `canada` Shopify Market context. Otherwise the global channel setting applies to every market and a single toggle covers all of them — record which behavior the UI exposes.
...
If the toggle does not exist for this market, STOP. Fall back to Path B (custom GraphQL feed generator) for `canada` only — see `../scripts/generate_pinterest_feed_grouped.py` and upload its `canada` output as a separate Pinterest catalog source.
...
1. New Pinterest catalog item count for the `canada` source.
...
7. Run `python3.13 ops/scripts/check_pinterest_feed_grouping.py --market canada` -> expect `PASS`.

## Rollback

If after-state readback shows fewer working products than expected, missing hero images, or new disapprovals:

1. Return to the same Shopify Pinterest channel settings page (in the `canada` context if per-market).
...
- Currency is `CAD`, so wasted impressions on duplicate variant items burn `CAD` budget directly.
- Languages in scope are `en, fr` — every language route emits the same duplicated shape today; the fix applies uniformly.
...
## Acceptance Criteria for `canada`

- Catalog rows for `canada` collapse from ~20x variant inflation to one row per parent OR rows sharing `item_group_id`.
- `image_link` on every row is the parent product's featured image.
- `additional_image_link` carries variant images.
- Guardrail `check_pinterest_feed_grouping.py --market canada` returns PASS.
- Pinterest catalog shows no new severity-1 disapprovals for `canada`.
