# Pinterest Feed Grouping Approval Packet — United States

Shopify Market handle: `us`
Owner priority rank: 1
Languages: en, es
...
1. Pinterest Ads Manager -> Catalogs -> advertiser `549756244483` -> catalog item count for the `us` source (or the merged catalog if Pinterest does not split by Shopify Market).
2. Pinterest "Dresses" group entry count where source/market = `us` (if Pinterest exposes per-source filters; otherwise capture overall).
...
6. Confirm the connected advertiser ID matches `549756244483` and the Shopify Market mapping is `us`.
...
2. If the channel supports per-market overrides, select the `us` Shopify Market context. Otherwise the global channel setting applies to every market and a single toggle covers all of them — record which behavior the UI exposes.
...
If the toggle does not exist for this market, STOP. Fall back to Path B (custom GraphQL feed generator) for `us` only — see `../scripts/generate_pinterest_feed_grouped.py` and upload its `us` output as a separate Pinterest catalog source.
...
1. New Pinterest catalog item count for the `us` source.
...
7. Run `python3.13 ops/scripts/check_pinterest_feed_grouping.py --market us` -> expect `PASS`.

## Rollback

If after-state readback shows fewer working products than expected, missing hero images, or new disapprovals:

1. Return to the same Shopify Pinterest channel settings page (in the `us` context if per-market).
...
## Why This Matters for `United States`

- Currency is `USD`, so wasted impressions on duplicate variant items burn `USD` budget directly.
- Languages in scope are `en, es` — every language route emits the same duplicated shape today; the fix applies uniformly.
...
- Catalog rows for `us` collapse from ~20x variant inflation to one row per parent OR rows sharing `item_group_id`.
- `image_link` on every row is the parent product's featured image.
- `additional_image_link` carries variant images.
- Guardrail `check_pinterest_feed_grouping.py --market us` returns PASS.
- Pinterest catalog shows no new severity-1 disapprovals for `us`.
