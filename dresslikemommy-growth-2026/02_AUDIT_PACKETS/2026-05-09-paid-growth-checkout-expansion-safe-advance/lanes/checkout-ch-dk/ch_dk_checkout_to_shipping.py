#!/usr/bin/env python3
"""CH/DK isolated-browser checkout-to-shipping QA.
...
SUMMARY_PATH = LANE_DIR / "ch_dk_checkout_to_shipping_summary.json"
REPORT_PATH = LANE_DIR / "CH_DK_CHECKOUT_TO_SHIPPING.md"
...
def checkout_state(client: CdpClient, currency: str) -> dict[str, Any]:
    expression = r"""
        (() => {
          ...
          const moneyPattern = new RegExp('(' + currency + '|CHF|DKK|kr|EUR|USD|GBP|CAD|AUD|\\\\$|free|gratis)', 'i');
          ...
          return {
            ...
            has_currency: new RegExp('\\\\b' + currency + '\\\\b|CHF|DKK|kr|\\\\$\\\\s*\\\\d|€\\\\s*\\\\d|£\\\\s*\\\\d', 'i').test(text),
...
def build_summary(client: CdpClient, country_code: str) -> dict[str, Any]:
    ...
    if not isinstance(state, dict) or state.get("runtime_exception"):
        ...
        state = {
            ...
            "has_currency": bool(re.search(r"\\b(CHF|DKK)\\b|\\bkr\\b|\\$\\s*\\d", fallback.get("text", ""), re.I)),
    ...
    return {
        ...
        "lane": "CH/DK isolated-browser checkout-to-shipping QA",
...
def write_report(summaries: list[dict[str, Any]]) -> None:
    lines = [
        "# CH/DK Checkout To Shipping Readback",
        "",
...
def main() -> None:
    ...
    parser.add_argument("--countries", nargs="+", choices=sorted(MARKETS), default=["CH"])
