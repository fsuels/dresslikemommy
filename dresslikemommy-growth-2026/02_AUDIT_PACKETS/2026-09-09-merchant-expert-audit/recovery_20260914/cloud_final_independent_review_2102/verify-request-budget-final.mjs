...
const reviewed=JSON.parse(await readFile('/Users/fsuels/Projects/dresslikemommy/dresslikemommy-growth-2026/02_AUDIT_PACKETS/2026-09-09-merchant-expert-audit/recovery_20260911/direct-feed-deployment/automation_release_v1/config.json','utf8'));
const splitBase=JSON.parse(env.MERCHANT_CONFIG_JSON);splitBase.eligibilityHoldsExternal=true;
env.MERCHANT_CONFIG_JSON=JSON.stringify(splitBase);env.MERCHANT_ELIGIBILITY_HOLDS_JSON=JSON.stringify(reviewed.eligibilityHolds);
...
try {
 ...
 externalFetches:perConsumer.authFetches+perConsumer.adminFetches,checkpointed:true,finalSplitConfig:true,realNetworkCalls:0,livePlanReadback:false,
 ...
 await writeFile(new URL('request-budget-final.json',import.meta.url),JSON.stringify(receipt,null,2)+'\n');
