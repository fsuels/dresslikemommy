"""Incident-reconciled Navy guard: preserves original guard and permits one proven recreated title metadata identity."""
import copy
import hashlib
import json
...
if phase == 'source-prewrite':
    ...
    english = read(PRIVATE / 'after_english_complete.json')
    recovery = read(ROOT / 'seo_metadata_recovery_verification.json')
    assert recovery['status'] == 'VERIFIED_TITLE_METADATA_AND_FLAGS_RESTORED'
    assert recovery['privateAfterSha256'] == sha(PRIVATE / 'after_english_complete.json')
    assert recovery['newTitleTagId'] == 'gid://shopify/Metafield/38753913929825'
    ...
    old_meta = {(f['namespace'], f['key']): f for f in expected['product']['metafields']['nodes']}
    actual_meta = english['data']['product']['metafields']['nodes']
    assert len(old_meta) == len(actual_meta) == 27
    assert set(old_meta) == {(f['namespace'], f['key']) for f in actual_meta}
    mirror_count = title_count = 0
    for actual in actual_meta:
        key = (actual['namespace'], actual['key'])
        old = old_meta[key]
        if key == ('global', 'description_tag'):
            assert actual['id'] == old['id']
            assert actual['value'] == candidate[('en', 'meta_description')]
            assert actual['compareDigest'] and actual['compareDigest'] != old['compareDigest']
            old['value'], old['compareDigest'] = actual['value'], actual['compareDigest']
            mirror_count += 1
        elif key == ('global', 'title_tag'):
            assert old['id'] == 'gid://shopify/Metafield/38482068078689'
            assert actual['id'] == 'gid://shopify/Metafield/38753913929825'
            old['id'] = actual['id']
            title_count += 1
        assert old == actual, 'Unexpected metadata delta: ' + str(key)
    assert mirror_count == title_count == 1
    # The recreated title record moves to the connection end; complete key/id/value equality above is required.
    expected['product']['metafields']['nodes'] = [old_meta[(f['namespace'], f['key'])] for f in actual_meta]
    assert english['data']['product']['metafields']['pageInfo']['hasNextPage'] is False
    expected['product']['metafields']['pageInfo']['endCursor'] = english['data']['product']['metafields']['pageInfo']['endCursor']
    ...
    report.update(englishFieldsVerified=2, sourceDigestsRefreshed=2, protectedVariantRecordsUnchanged=24, protectedOtherMetafieldsUnchanged=25, originalTitleValueTypeAndDigestRestored=True, titleTagIdentityRecreated=True, existingTranslationValuesUnchangedAfterEnglish=100, sourceAfterCompletedAt=english['completedAt'], privateAfterEnglishSha256=sha(PRIVATE / 'after_english_complete.json'))
...
(ROOT / (phase + '_restored_verification.json')).write_text(json.dumps(report, indent=2) + '\n')
