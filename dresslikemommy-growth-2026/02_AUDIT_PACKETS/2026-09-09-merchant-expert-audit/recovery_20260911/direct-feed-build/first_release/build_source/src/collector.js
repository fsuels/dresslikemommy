import { IDENTITY_QUERY, PRODUCTS_QUERY, VARIANTS_QUERY, MANIFEST_QUERY } from './queries.js';
...
export async function collectCatalog(graphql, config, market, { now = () => new Date(), progress = () => {} } = {}) {
  ...
  const variables = { country: market.country, locale: market.locale, marketId: market.marketId ?? null, onlinePublicationId: config.onlinePublicationId };
  ...
  const finalManifest = await paginate(
    after => graphql(MANIFEST_QUERY, { country: market.country, onlinePublicationId: config.onlinePublicationId, after }),
