...
export async function buildFeed(snapshot, config, market, { now = new Date(), previousIds = [] } = {}) {
  ...
  for (const product of snapshot.products) {
    if (!product.onlinePublished || !product.countryPublished) {
      exclusions.push({ productId: product.id, variants: product.variants.length, code: !product.onlinePublished ? 'not_on_online_store' : 'not_published_in_country' }); continue;
    }
    ...
    let content;
    try { content = localizedContent(product, config, market); } catch (e) { errors.push({ productId: product.id, code: e.code || 'invalid_product_content' }); continue; }
    for (const variant of product.variants) {
      ...
      try {
        ...
        assert(content.title && content.description, 'missing_title_or_description');
        ...
        const row = {
          ...
          title: [...content.title].slice(0, 150).join(''), description: [...content.description].slice(0, 5000).join(''),
          link: landingLink(product, variant, content, market), image_link: imageUrl,
