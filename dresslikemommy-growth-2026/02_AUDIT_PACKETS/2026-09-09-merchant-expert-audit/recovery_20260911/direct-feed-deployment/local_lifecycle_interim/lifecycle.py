...
SOURCE_QUALIFICATION = BASE / 'international_candidate/au-en.full_cohort_qualification_20260911.json'
PROTECTED_US_IDS = (
...
def contain(run, hold_path=None, revise=False):
    ...
    output, manifest, diagnostics = apply_holds(raw_body, raw_manifest,
        ...
        protected_ids=PROTECTED_US_IDS,
        source_parent_ids=[p['id'] for p in read(original / 'us-en.snapshot.json')['products']])
...
def verify_containment(run):
    ...
    output, manifest, diagnostics = apply_holds((raw / 'us-en.tsv').read_bytes(), read(raw / 'us-en.manifest.json'),
        ...
        protected_ids=PROTECTED_US_IDS,
        source_parent_ids=[p['id'] for p in read(raw / 'us-en.snapshot.json')['products']])
...
def candidate(run):
    ...
    qualification_sha = read(run / 'eligibility_holds.json')['source_qualification']['sha256']
    require(sha(SOURCE_QUALIFICATION.read_bytes()) == qualification_sha, 'source_qualification_changed')
...
def validate_review(run, review_path, expected_hash):
    ...
    require(sha(HOLD_SPEC.read_bytes()) == packet['eligibility_holds_sha256'], 'active_hold_spec_changed')
    ...
    fields = ('eligibility_holds_sha256', 'source_qualification_sha256', 'cohort_filter_sha256', 'containment_receipt_sha256',
