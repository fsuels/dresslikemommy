...
ROOT = BASE.parents[4]
REVIEWER = '/claude/merchant_us_restore_review_20260925'
checks, failures = [], []
...
ENTITIES = {'amp': '&', 'lt': '<', 'gt': '>', 'quot': '"', 'apos': "'", 'nbsp': '\u00a0', 'ndash': '\u2013', 'mdash': '\u2014', 'rsquo': '\u2019', 'lsquo': '\u2018', 'ldquo': '\u201c', 'rdquo': '\u201d', 'hellip': '\u2026', 'copy': '\u00a9', 'reg': '\u00ae', 'trade': '\u2122', 'euro': '\u20ac', 'pound': '\u00a3', 'yen': '\u00a5', 'middot': '\u00b7', 'times': '\u00d7', 'bull': '\u2022'}
JS_WS = '[\t\n\v\f\r \u00a0\u1680\u2000-\u200a\u2028\u2029\u202f\u205f\u3000\ufeff]+'
SUPPLIER = re.compile(r'(?:alibaba\.com|aliexpress\.com|1688\.com|taobao\.com|tmall\.com)', re.I)


def clean_text(value):
    def entity(m):
        numeric, named = m.group(1), m.group(2)
        if named:
            return ENTITIES.get(named.lower(), m.group(0))
        n = int(numeric[1:], 16) if numeric[0].lower() == 'x' else int(numeric)
        return chr(n) if 0 < n <= 0x10ffff and not (0xd800 <= n <= 0xdfff) else ' '
    text = re.sub(r'&#(x[0-9a-f]+|\d+);|&([a-z]+);', entity, str(value or ''), flags=re.I)
    text = re.sub('[\u0000-\u001f\u007f-\u009f\u200b-\u200f\u202a-\u202e\u2066-\u2069\ufeff]', ' ', text)
    return re.sub(JS_WS, ' ', text).strip(' ')
...
for p in products:
    ...
    for v in p['variants']:
        ...
        check('description_equals_source_text:' + vid, row['description'] == clean_text(clean_text(p['description'])[:5000]))
        check('no_supplier_domain:' + vid, not any(SUPPLIER.search(value or '') for value in row.values()))
...
check('bytes_changed_flag_consistent', ((RUN / 'candidate/us-en.tsv').read_bytes() == (RUN / 'before.tsv').read_bytes()) == packet['bytes_unchanged'])
changed_ids = sorted(delta['changed_fields'])
check('changed_rows_description_only', all(delta['changed_fields'][key] == ['description'] for key in changed_ids))
for key in changed_ids:
    pid, vid = key.split('_')[2], key.split('_')[3]
    p, v = eligible[key]
    check('changed_description_equals_snapshot_source:' + vid, by_id[key]['description'] == clean_text(clean_text(p['description'])[:5000]) and by_id[key]['description'] != before_by_id[key]['description'])
check('no_added_buyer_routes_needed', delta['added_ids'] == [])
check('no_removed_offers', delta['removed_ids'] == [])
...
summary = {
    ...
    'changed_description_rows': len(delta['changed_fields']), 'changed_description_parents': len({k.split('_')[2] for k in delta['changed_fields']}),
    'limits': ['Existing buyer-route limitations are not cured by this source renewal; the only row changes are Shopify-sourced descriptions.', str(len(diagnostics['warnings'])) + ' existing nonblocking diagnostics remain; no claim of perfect attributes or sales/profit.'],
}
...
if not failures:
    ...
    review.update(decision='APPROVE_EXACT_US_FEED_POINTER', independent=True, reviewer=REVIEWER,
        ...
        approved_action='Owner-directed Sep 24 feed restoration: existing US/en/USD free-listing source pointer renewal only',
