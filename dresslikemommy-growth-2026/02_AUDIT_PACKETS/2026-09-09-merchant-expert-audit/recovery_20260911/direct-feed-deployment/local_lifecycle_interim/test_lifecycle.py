...
class PublisherTests(unittest.TestCase):
    def setUp(self):
        ...
        self.active_holds = self.run / 'active-holds.json'
        self.source_gate = self.run / 'source-gate.json'; self.source_gate.write_bytes(b'{}')
        self.active_holds.write_bytes(app.encode({'schemaVersion': 1, 'markets': ['us-en'], 'holds': [],
            'source_qualification': {'sha256': app.sha(self.source_gate.read_bytes())}}))
        for name, value in [('HOLD_SPEC', self.active_holds), ('SOURCE_QUALIFICATION', self.source_gate),
                            ('PROTECTED_US_IDS', ('shopify_US_1_2',))]:
    ...
    def test_review_binds_candidate_and_lifecycle_authority(self):
        packet = {key: 'fixture' for key in ('eligibility_holds_sha256', 'cohort_filter_sha256',
            'source_qualification_sha256', 'containment_receipt_sha256', 'prepare_receipt_sha256', 'live_build_receipt_sha256',
        ...
        packet['eligibility_holds_sha256'] = app.sha(self.active_holds.read_bytes())
    ...
    def test_changed_hold_file_invalidates_containment(self):
        ...
        changed['scope_note'] = 'Changed fixture'; path.write_bytes(app.encode(changed))
    ...
    def test_reviewed_local_hold_revision_preserves_prior_evidence(self):
        ...
        revised['scope_note'] = 'Reviewed fixture clarification'
    ...
    def test_contain_wrapper_cannot_remove_protected_offer_with_other_rows_remaining(self):
        self.source_fixture()
        second = feed('10.00 USD').splitlines(keepends=True)[1].replace(b'shopify_US_1_2', b'shopify_US_3_4').replace(b'shopify_US_1', b'shopify_US_3').replace(b'variant=2', b'variant=4')
        self.body += second
        self.after = manifest(self.body, '2026-09-11T11:00:00Z')
        self.after.update(rows=2, rowIds=['shopify_US_1_2', 'shopify_US_3_4'], sourceParents=2, sourceVariants=2)
        self.write_candidate()
        path = self.run / 'candidate/us-en.snapshot.json'; source = app.read(path)
        source['products'].append({'id': 'gid://shopify/Product/3', 'variants': [{'id': 'gid://shopify/ProductVariant/4'}]})
        source['activeCount'] = source['finalActiveCount'] = {'count': 2, 'precision': 'EXACT'}
        path.write_bytes(app.encode(source))
        holds = app.read(self.active_holds)
        holds['holds'] = [{'product_id': 'gid://shopify/Product/1', 'reason': 'Fixture conflict',
                           'evidence': ['fixture'], 'release_condition': 'Reviewed fixture resolution'}]
        self.active_holds.write_bytes(app.encode(holds))
        with self.assertRaisesRegex(ValueError, 'protected_offer_lost'):
            app.contain(self.run)
        self.assertFalse((self.run / 'contain.receipt.json').exists())

    def test_replay_wrapper_enforces_protected_ids(self):
        self.source_fixture(); app.contain(self.run)
        with patch.object(app, 'PROTECTED_US_IDS', ('shopify_US_1_2', 'shopify_US_3_4')):
            with self.assertRaisesRegex(ValueError, 'protected_offer_lost'):
                app.verify_containment(self.run)

    def test_active_hold_changed_during_upload_never_commits_pointer(self):
        self.source_fixture(); app.contain(self.run)
        def changed():
            holds = app.read(self.active_holds); holds['scope_note'] = 'New active review'
            self.active_holds.write_bytes(app.encode(holds))
        self.api.on_object_put = changed
        with patch.object(app, 'now', return_value=dt.datetime(2026, 9, 11, 11, 2, tzinfo=dt.timezone.utc)):
            packet = app.candidate(self.run)
            review = {**packet, 'decision': 'APPROVE_EXACT_US_FEED_POINTER', 'independent': True,
                      'reviewer': 'independent-reviewer', 'source_eligibility_and_all_prices_verified': True,
                      'single_writer_claim_verified': True, 'source_eligibility_holds_verified': True}
            path = self.run / 'independent-review.json'; path.write_bytes(app.encode(review))
            with self.assertRaisesRegex(app.Stop, 'active_hold_spec_changed'):
                app.promote(self.api, self.run, path, app.sha(path.read_bytes()))
        self.assertEqual(self.api.writes, [self.after['objectKey']])
        self.assertEqual(self.api.objects[app.POINTER], app.encode(self.before))

    def test_changed_source_qualification_invalidates_candidate(self):
        self.source_fixture(); app.contain(self.run)
        self.source_gate.write_bytes(b'{"changed": true}')
        with self.assertRaisesRegex(app.Stop, 'source_qualification_changed'):
            app.candidate(self.run)
