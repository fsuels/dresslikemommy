...
TITLE = "Green Botanical Ruffle Mommy and Me Dresses"
SEO_TITLE = "Botanical Mommy and Me Dresses | Dress Like Mommy"
SEO_DESCRIPTION = "Green botanical mommy-and-me dresses for mom + daughter with airy ruffles. Size rows cover Child 4Y-10Y and Mother S-XL."
PRINT_NAME = "Green Botanical Ruffle"
SHORTCODE = "GBRF"
...
COLORWAYS = [
    {"name": "Green Botanical", "token": "GREEN", "gid": "gid://shopify/Metaobject/70220546145"},
]
...
SCRIPT_PATH = ROOT / "ops/scripts/create-gbrf-green-botanical-ruffle-mommy-and-me-dresses.sh"
...
SIZE_CHART = [
    {
        ...
        "skirt_cm": 75,
        "pant_cm": "-",
        "source_note": "Shoulder, bust, and garment length transcribed from attached child chart; waist derived as chest and hip derived as chest + 4 because the child dress chart omits waist and hip."
    },
    {
        "audience": "child",
        "role": "Girl Dress",
    ...
    },
    {
        ...
        "skirt_cm": 124,
        "pant_cm": "-",
        "source_note": "Shoulder, bust, and garment length transcribed from attached adult chart; hip derived as bust + 6 and waist derived as hip - 8 because the mother dress chart omits waist and hip."
    },
    {
        "audience": "mother",
        "role": "Mother Dress",
...
def build_body() -> str:
    ...
    return "\n".join(
        [
            '<ul>',
            '<li><strong>Fabric:</strong> Lightweight woven-look fabric shown in the supplied image; exact fiber content was not visible from the evidence.</li>',
            ...
            '<li><strong>Print:</strong> Green Botanical Ruffle pairs a soft ivory base with an allover green leafy pattern.</li>',
            '<li><strong>Design details:</strong> Ruffled neckline, tie-front detail, relaxed waist, sleeveless shape, and airy tiered skirt movement for mother and child.</li>',
            ...
            *rendered_rows,
            ...
            '</table>',
            '',
            '<p>The Green Botanical Ruffle Mommy and Me Dresses listing is built for a coordinated mom-and-daughter beach or resort moment without turning the outfit into a bundled set. Choose the child dress size, choose the mother dress size, and keep the same leafy green ruffle look across both pieces.</p>',
            '',
            '<p>The attached size chart is the variant source of truth. It gives shoulder, bust, and garment length measurements; waist and hip values are derived and documented because the chart does not publish waist or hip columns.</p>',
            '',
            '<h3>Key Features:</h3>',
            '<ul>',
            '<li><strong>Mom + daughter match:</strong> Coordinated green botanical dresses across child and mother sizes.</li>',
            '<li><strong>Vacation-ready ruffles:</strong> Ruffled neckline and light tiering frame beach, resort, and family-photo outfits.</li>',
            '<li><strong>Fresh botanical palette:</strong> Green leafy artwork over a light base keeps the look bright and easy to style.</li>',
...
def run_variant_model_guard(variants: list[dict]) -> None:
    with tempfile.TemporaryDirectory() as tmp:
        ...
        evidence_path.write_text(
            json.dumps({"notes": "Attached evidence shows one green botanical ruffle dress garment for mother and girl. The requested scope is one dress colorway only."}),
            encoding="utf-8",
        )
        subprocess.run(
            [
...
def metafields(product_id: str) -> list[dict]:
    ...
    return [
        ...
        {"ownerId": product_id, "namespace": "mm-google-shopping", "key": "custom_label_1", "type": "single_line_text_field", "value": "Green Botanical"},
...
def upload_media(product_id: str) -> None:
    ...
    alt_by_name = {
        "01-green-botanical-ruffle-mommy-and-me-dresses.png": "Mother and daughter wearing matching green botanical ruffle dresses on the beach.",
    }
    for path in sorted(UPLOAD_DIR.iterdir()):
        ...
        if "size-chart" in path.name:
            continue
        alt = alt_by_name.get(path.name, "Green botanical ruffle mommy and me dress product image.")
