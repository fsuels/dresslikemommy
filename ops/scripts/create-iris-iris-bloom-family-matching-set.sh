...
HANDLE="iris-bloom-family-matching-set"
TITLE="Iris Bloom Family Matching Set — Dress & Shirt"
SEO_TITLE="Iris Bloom Family Set | Dress Like Mommy"
SEO_DESCRIPTION="Lightweight woven family matching set with iris-print dresses and shirts for moms, dads, girls & boys. Sizes 1-2Y-10Y, Mother S-2XL, Father S-4XL."
PRINT_NAME="Iris Bloom"
SHORTCODE="IRIS"
COLOR_TOKEN="IRIS"
COLOR_NAME="Iris Bloom"
...
SEASON="Summer"
VENDOR_URL="https://detail.1688.com/offer/813439102302.html"
VENDOR="dresslikemommy.com"
FORCE_SPEC_PRICES="true"
CHILD_PRICE="31.99"
MOTHER_PRICE="36.99"
PRICE_NEIGHBOR_HANDLE="ocean-marble-family-matching-set"
SIZE_NEIGHBOR_HANDLE="citrus-bloom-family-matching-set"

SCRIPT_PATH="${ROOT}/ops/scripts/create-iris-iris-bloom-family-matching-set.sh"
...
body_html = "\n".join([
    ...
    li("Print", f"\"{print_name}\" pairs oversized red, blue, and yellow iris flowers with leafy green stems on a pale ground for a bright resort-ready palette."),
    li("Design details", "Girls and moms wear the coordinating sleeveless dress with blue shoulder straps and an easy full silhouette, while boys and dads get the matching collared button-front shirt with short sleeves. Shorts, sandals, sunglasses, hats, handbags, baskets, and other accessories shown in the photos are styling only and not included."),
    ...
    *shirt_rows,
    ...
    "</table>",
    "",
    "<p>Iris Bloom gives family matching a bright vacation feel with oversized red, blue, and yellow flowers across a pale woven-look ground. Moms and girls get the coordinating sleeveless dress with blue shoulder straps and an easy full silhouette, while dads and boys wear the matching collared short-sleeve shirt in the same floral story.</p>",
    "",
    ...
    li("Iris bloom palette", "Oversized iris-style flowers in red, blue, and yellow with leafy green stems create a vivid matching look that photographs beautifully."),
    ...
    li("Styling pieces not included", "Shorts, sandals, sunglasses, hats, handbags, baskets, and other accessories shown in the photos are styling only and were intentionally excluded from this listing."),
    "</ul>",
    "",
...
METAFIELDS_JSON="$(jq -nc \
  ...
  [
      ...
      {ownerId: $pid, namespace: "custom", key: "pattern", type: "single_line_text_field", value: "Iris Bloom"},
      ...
      {ownerId: $pid, namespace: "mm-google-shopping", key: "custom_label_1", type: "single_line_text_field", value: "Iris Bloom"},
...
for image_path in "${MEDIA_FILES[@]}"; do
  ...
  case "$image_name" in
    01-*)
      ...
      ;;
    *)
      alt_text="Iris Bloom matching dress and short-sleeve shirt."
      ;;
  esac
...
for recap in derived["recap"]:
    ...
    put("Google Shopping / Custom Label 1", "Iris Bloom")
    ...
    put("Pattern (product.metafields.custom.pattern)", "Iris Bloom")
    ...
    put("Color (product.metafields.shopify.color-pattern)", "White, Blue, Red, Yellow, Green, Floral, Multicolor")
...
lines.append("| SHORTCODE | auto → `IRIS` |")
lines.append("| COLOR_TOKEN | auto → `IRIS` |")
lines.append("| FORCE_SPEC_PRICES | true |")
lines.append("")
lines.append("## Vendor fetch status")
lines.append("The direct 1688 page returned Alibaba anti-bot/captcha markup during this run, so the attached vendor chart and supplied family photos were used as the authoritative source of truth. The supplied chart is a body size reference chart: it publishes size, recommended height, and recommended weight only. It does not publish garment chest, waist, hip, skirt length, sleeve, shoulder, pant, or garment-length measurements, so those unavailable size-table cells are rendered as `—` instead of being guessed. The infant crawler table was excluded because the request asked for `dress, shirt`. Styling-only shorts, sandals, sunglasses, hats, handbags, baskets, and accessories were excluded because this request only asked for the shirt and dress designs. Pricing was anchored to recent family matching set precedence with FORCE_SPEC_PRICES, and size metaobject GIDs were anchored to `citrus-bloom-family-matching-set`. The Shopify taxonomy stays `Outfit Sets` for honest standard-category attributes.")
lines.append("")
...
lines.append("- The vendor chart provides body height and weight guidance only; garment chest, waist, hip, length, skirt, sleeve, shoulder, and pant measurements are unavailable and rendered as `—` rather than guessed.")
...
lines.append("- Waist and hip derivation could not be applied because the source chart does not provide chest/bust values; this is documented as a source limitation.")
lines.append("- Adult height values are included from the adult reference chart; infant crawler rows were excluded because they are outside the requested dress/shirt designs.")
lines.append("- Styling-only shorts, sandals, sunglasses, hats, handbags, baskets, and accessories shown in the supplied photos were intentionally excluded because they fall outside this `Dress` + `Shirt` listing request.")
...
lines.append("- Adult dress S / M / L / XL / 2XL → Mother S / M / L / XL / 2XL")
lines.append("- Adult shirt S / M / L / XL / 2XL / 3XL / 4XL → Father S / M / L / XL / 2XL / 3XL / 4XL")
lines.append("")
lines.append("### EXCLUDE_ITEMS decisions")
lines.append("- No explicit `EXCLUDE_ITEMS` string was supplied.")
lines.append("- The shorts, sandals, sunglasses, hats, handbags, baskets, and accessories shown in the supplied photos are styling only and were excluded because the request explicitly limited the listing to the shirt and dress designs.")
lines.append("")
...
lines.append("- If the vendor page becomes directly readable later, look for any garment-measurement chart that may exist beyond the supplied body reference chart.")
...
lines.append("- Size guidance anchor: attached body reference chart; no garment grading chart was available in the supplied evidence")
