...
HANDLE = "midnight-paint-splash-family-matching-tops"
TITLE = "Midnight Paint Splash Family Matching Tops — Button-Up Shirt"
SEO_TITLE = "Midnight Paint Family Shirts | Dress Like Mommy"
PRINT_NAME = "Midnight Paint Splash"
SHORTCODE = "MPSH"
COLOR_TOKEN = "MIDSPT"
COLOR_NAME = "Midnight Paint Splash"
...
SCRIPT_PATH = ROOT / "ops/scripts/create-mpsh-midnight-paint-splash-family-matching-tops.sh"
SOURCE_SIZE_CHART = ROOT / "ops/listings/source-size-chart-family-matching-casual-shirts.png"
PRODUCT_IMAGE = UPLOAD_DIR / "01-midnight-paint-splash-shirt.png"
...
SEO_DESCRIPTION = (
    "Midnight Paint Splash shirts for mom, dad, girls & boys in lightweight "
...
def build_body() -> str:
    ...
    return "\n\n".join([
        ...
        "<li><strong>Print reference:</strong> Midnight Paint Splash layers vivid blue, teal, red, orange, and yellow paint-splash motifs over a dark navy and black ground.</li>",
        "<li><strong>Design details:</strong> Collared neckline, button-up front, short sleeves, a chest pocket, and one shared print across child and adult sizes. The white undershirt and hanger shown are styling or display pieces only and are not included.</li>",
        ...
        *rendered,
        "</tbody></table>",
        "<p>Midnight Paint Splash brings expressive color to family matching with a dark navy and black base scattered in vivid blue, teal, red, orange, and yellow paint-like marks. The open-collar button-up silhouette makes the energetic print easy to style over a plain undershirt or wear on its own for coordinated outings and family portraits.</p>",
        ...
        "<h3>Key Features:</h3>",
        "<ul>",
        "<li><strong>One coordinated shirt:</strong> The same collared short-sleeve button-up look with a chest pocket is offered across child and adult sizes.</li>",
        "<li><strong>Expressive abstract print:</strong> A dark ground and bright multicolor paint-splash motifs create a bold, photo-ready family palette.</li>",
        ...
        "</ul>",
        "<p>Choose the child and adult sizes you need to create a coordinated Midnight Paint Splash look for the next colorful family memory.</p>",
    ])
...
def metafields(product_id: str) -> list[dict]:
    ...
    return [
        ...
        {"ownerId": product_id, "namespace": "custom", "key": "style", "type": "single_line_text_field", "value": "Abstract Paint-Splash Button-Up Shirt"},
...
def validate_preflight(body: str, variants: list[dict]) -> None:
    ...
    for row, variant in zip(SIZE_CHART, variants):
        ...
        if not variant["inventoryItem"]["sku"].startswith("DLM-MPSH-"):
            errors.append("SKU prefix is not DLM-MPSH")
...
def upload_media(product_id: str) -> None:
    ...
    path = PRODUCT_IMAGE
    alt = "Midnight Paint Splash short-sleeve button-up shirt with a chest pocket and bold multicolor paint-splash motifs on a dark ground."
...
def write_csv(body: str, variants: list[dict]) -> None:
    ...
    for i, (row, variant) in enumerate(zip(SIZE_CHART, variants), start=1):
        ...
        values.update({
            ...
            "Style (product.metafields.custom.style)": "Abstract Paint-Splash Button-Up Shirt" if i == 1 else "",
            ...
            "Color (product.metafields.shopify.color-pattern)": "Blue, Multicolor" if i == 1 else "",
...
def write_listing(product_id: str, verify: dict, variants: list[dict], price_rows: list[dict]) -> None:
    ...
    skipped = [
        ...
        ("shopify.color-pattern Teal/Red/Orange/Yellow/Black", "The visible colors remain in copy and tags; only verified Blue and Multicolor catalog references are written."),
    ...
    lines = [
        ...
        "| DESIGNS_TO_LIST | one Midnight Paint Splash short-sleeve button-up shirt |",
        "| FORCE_SPEC_PRICES | true |",
        ...
        *recap, "",
        "## Derivations",
        "- The supplied product image supports one unisex collared, short-sleeve button-up shirt with a dark navy and black ground, vivid blue, teal, red, orange, and yellow paint-splash motifs, and a chest pocket. The white undershirt and hanger are not included.",
...
if __name__ == "__main__":
    main()
PY
/usr/bin/python3 "$ROOT/ops/scripts/finalize_shopify_listing_localization.py" --handles "midnight-paint-splash-family-matching-tops"
