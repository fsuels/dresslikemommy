...
#
# Usage:  bash create-vcf-peter-rabbit-mommy-and-me-pajamas.sh
...
# -------------------------------------------------------------------
PRODUCT_TITLE='Peter Rabbit Mommy and Me Pajamas — Short-Sleeve Set'
PRODUCT_HANDLE='peter-rabbit-mommy-and-me-pajamas'
SEO_TITLE='Peter Rabbit Mommy & Me Pajamas | Dress Like Mommy'
SEO_DESC='Shop our Peter Rabbit matching mommy-and-me pajamas — soft cotton short-sleeve sets for mom + daughter. Sizes 3Y, 5Y, 8Y & Mom One Size.'
COLOR_NAME='Peter Rabbit'
COLOR_TOKEN='CREAM'
SHORTCODE='VCF'
VENDOR_URL='https://detail.1688.com/offer/792917229223.html'
DESC_HTML_FILE="$SCRIPT_DIR/peter-rabbit-description.html"
...
# 4. description file exists
if not os.path.isfile(desc_path): errors.append(f"missing desc file: {desc_path}")
# 5. body size-table row count must == SIZE_CHART length (only count tbody rows)
if os.path.isfile(desc_path):
    html = open(desc_path).read()
    # count rows inside size-chart tbody
...
# Emit derived lists (Size option values, tags, SKUs, shopify.size GIDs, size phrase)
derived = {
...
tags += derived["child_buckets"]        # mother tags only for mother rows that exist
...
echo "$PRODUCT_ID" > "$SCRIPT_DIR/.peter-rabbit-last-product-id"
...
# Store catalog GIDs from neighbors (validated via metaobject query)
# color-pattern: Beige + Floral + Multicolor
color_pattern_gids = [
...
# fabric: Cotton
fabric_gids = ["gid://shopify/Metaobject/69622399073"]
# target-gender: Female
target_gender_gids = ["gid://shopify/Metaobject/129971617889"]
# age-group: Toddlers + Kids + Adults (covers 3Y/5Y child-wear + mother)
age_group_gids = [
...
# shopify.size — only labels with a catalog entry (Mother One Size skipped)
...
mfs = [
    ...
    ["custom","pattern",           "single_line_text_field","Watercolor Peter Rabbit bunnies + meadow floral print"],
...
echo "$VER" > "$SCRIPT_DIR/.peter-rabbit-verify.json"
