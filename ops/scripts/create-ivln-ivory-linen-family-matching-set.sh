...
HANDLE = "ivory-linen-family-matching-set"
TITLE = "Ivory Linen Family Matching Set - Vest, Shirt & Shorts"
SEO_TITLE = "Ivory Linen Family Matching Set | Dress Like Mommy"
SEO_DESCRIPTION = "Ivory linen-look family matching separates for mom, dad, girls & boys. Vest, shirt, and shorts sizes 2Y-10Y, Mother S-3XL, Father S-4XL."
VENDOR_URL = "https://detail.1688.com/offer/1034249556745.html?"
...
SCRIPT_PATH = ROOT / "ops/scripts/create-ivln-ivory-linen-family-matching-set.sh"
...
girl_vest_rows = [(59,72),(62,76),(64,80),(67,84),(71,88),(74,92),(78,96)]
...
for (vendor, picker, age, height), (length, chest) in zip(child_rows, girl_vest_rows):
    add("child", "Girl Vest", "Vest", vendor, picker, age, "—", height, chest, length, source_note="Vendor girl table labels length/chest only; treated as vest per operator category and product image.")
...
for vendor, picker, weight, length, chest in mother_rows:
    add("mother", "Mother Vest", "Vest", vendor, picker, "—", weight, "—", chest, length, source_note="Vendor women's table omits waist/hip; 3XL is marked custom in source.")
...
role_tokens = {"Girl Vest": "GRL", "Mother Vest": "MOM", "Boy Shirt": "BOY", "Boy Shorts": "BOY", "Father Shirt": "DAD", "Father Shorts": "DAD"}
def sku(row):
    type_token = {"Vest": "VST", "Shirt": "SHRT", "Shorts": "SHRTS"}[row["garment"]]
...
body = "\n".join([
    "<ul>",
    "<li><strong>Fabric:</strong> Lightweight ivory linen-look woven fabric; exact fiber content was not visible from the blocked vendor page.</li>",
    "<li><strong>Family story:</strong> A clean family matching look for moms, dads, girls, and boys in soft vacation ivory.</li>",
    "<li><strong>Color:</strong> Solid ivory pieces designed to coordinate without a busy print.</li>",
    "<li><strong>Design details:</strong> Girls and moms wear the button-front vest; boys and dads wear the camp-collar shirt or matching shorts. Hats, jewelry, bags, sunglasses, and shoes are styling only.</li>",
    ...
    "</ul>",
    table([row for row in chart if row["garment"] == "Vest"], "Vest"),
    ...
    "<p>Ivory Linen keeps family dressing relaxed and bright, with separates that look polished for vacation photos, resort dinners, and sunny weekends. The vest option is chart-backed for girls and moms, while shirts and shorts are chart-backed for boys and dads.</p>",
    ...
    "<h3>Key Features:</h3>",
    "<ul>",
    "<li><strong>Three garment options:</strong> Vest, Shirt, and Shorts in one coordinated family listing.</li>",
    "<li><strong>Soft ivory palette:</strong> Easy to style for beach, cruise, and warm-weather family photos.</li>",
    ...
    "<li><strong>Lightweight feel:</strong> Linen-look texture gives the pieces an airy summer finish.</li>",
    "</ul>",
    "<p>Select each vest, shirt, or shorts size to create a crisp matching moment for the whole family.</p>",
])
...
tags = sorted(set([
    ...
    "Matching Family Vest", "Matching Family Shirt", "Matching Family Shorts", "Vest & Shirt & Shorts",
    "Sets", "Summer", "Beach", "Vacation", "Resort", "Cruise", "Ivory", "Cream", "White", "Solid",
    "Linen Look", "Button Vest", "Camp Collar Shirt", "Short Sleeve Shirt", "Shorts", "Four-Role Matching",
    "Girl Vest", "Mother Vest", "Boy Shirt", "Boy Shorts", "Father Shirt", "Father Shorts", VENDOR_URL,
...
metafields = [
    ...
    {"ownerId": product_id, "namespace": "custom", "key": "pattern", "type": "single_line_text_field", "value": "Solid Ivory"},
    ...
    {"ownerId": product_id, "namespace": "mm-google-shopping", "key": "custom_label_1", "type": "single_line_text_field", "value": "Ivory Linen"},
    {"ownerId": product_id, "namespace": "mm-google-shopping", "key": "custom_label_2", "type": "single_line_text_field", "value": "Summer"},
    {"ownerId": product_id, "namespace": "mm-google-shopping", "key": "custom_label_3", "type": "single_line_text_field", "value": "Vest Shirt Shorts"},
...
for image_path in sorted(list(UPLOAD_DIR.glob("*.png")) + list(UPLOAD_DIR.glob("*.jpg")) + list(UPLOAD_DIR.glob("*.jpeg")) + list(UPLOAD_DIR.glob("*.webp"))):
    alt = "Family wearing ivory linen-look matching vest, shirt, and shorts set." if "product" in image_path.name else "Ivory linen-look family matching set size chart."
...
row_count_ok = body.count("<tr>") - 3 == len(chart)
checks.append(("size table row count", row_count_ok, f"{body.count('<tr>') - 3} vs {len(chart)}"))
...
for item in recap:
    ...
    values = {
        ...
        "Google Shopping / Custom Label 0": "Family Matching", "Google Shopping / Custom Label 1": "Ivory Linen",
        "Google Shopping / Custom Label 2": "Summer", "Google Shopping / Custom Label 3": "Vest Shirt Shorts",
        "Google Shopping / Custom Label 4": "Four-Role Matching", "Category1 (product.metafields.custom.category1)": "Family Matching",
        "Pattern (product.metafields.custom.pattern)": "Solid Ivory", "Style (product.metafields.custom.style)": "Matching Family Set",
        ...
        "Age group (product.metafields.shopify.age-group)": "kids, adults", "Color (product.metafields.shopify.color-pattern)": "Ivory, White",
...
skipped = {
    ...
    "shopify.dress-style": "No dress is being sold in this listing; the chart-backed garment is a vest.",
    "shopify.fit": "No reliable store catalog value was confirmed for this specific linen-look separates fit.",
    "shopify.neckline": "Vest and camp-collar shirt necklines do not map to one honest product-level neckline.",
    "shopify.pants-length-type": "Shorts are included, but the product also sells vests and shirts, so one product-level pants length would be misleading.",
    "shopify.skirt-dress-length-type": "No skirt or dress garment is offered as a variant.",
    "shopify.sleeve-length-type": "Sleeveless vests and short-sleeve shirts share one product, so one sleeve value would be misleading.",
    "shopify.top-length-type": "The listing mixes vests, shirts, and shorts, so one top-length value is not honest for the whole product.",
}
lines = [
    f"# {TITLE}", "", "## Links",
    ...
    "| DESIGNS_TO_LIST | auto -> one ivory colorway from supplied imagery |",
    "| OPERATOR CATEGORY NOTE | one listing with Type options Vest, Shirt, Shorts |",
    "| FORCE_SPEC_PRICES | true |", "| SHORTCODE | auto -> `IVLN` |", "| COLOR_TOKEN | auto -> `IVORY` |", "",
    "## Vendor fetch status",
    "The direct 1688 page returned Alibaba anti-bot/captcha markup, so the attached size-chart image and supplied product photo were used as authoritative. The chart supports girl/mother vest rows, boy/father shirt rows, and boy/father shorts rows. Women/girls shorts appear in styling imagery but were not created as separate variants because no chart-backed shorts table was provided for those roles.",
    "",
    "## Option axes", "- Option 1: Type -> Vest, Shirt, Shorts", "- Option 2: Size -> role-bearing size labels",
...
lines += [
    ...
    "- Girl/mother vest waist and hip were derived where omitted by the vendor using the canonical top/vest rules.",
