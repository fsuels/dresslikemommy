...
HANDLE="garden-floral-family-matching-set"
TITLE="Garden Floral Family Matching Set — Dress & Shirt"
SEO_TITLE="Garden Floral Family Set | Dress Like Mommy"
SEO_DESCRIPTION="Lightweight woven family matching set with cream floral dresses and shirts for moms, dads, girls & boys. Sizes 1-2Y-10Y, Mother S-2XL, Father S-4XL."
PRINT_NAME="Garden Floral"
SHORTCODE="GFLR"
COLOR_TOKEN="CREAM"
COLOR_NAME="Garden Floral"
...
SEASON="Summer"
VENDOR_URL="https://detail.1688.com/offer/1046964144900.html"
...
SCRIPT_PATH="${ROOT}/ops/scripts/create-gflr-garden-floral-family-matching-set.sh"
...
body_html = "\n".join([
    ...
    li("Print", f"\"{print_name}\" layers oversized red and pink flowers, soft yellow accents, and green leaves over a cream ground for a bold resort-ready palette."),
    ...
    *shirt_rows,
    ...
    "</table>",
    "",
    "<p>Garden Floral gives family matching a polished vacation feel with red and pink florals on a soft cream ground. Moms and girls get the coordinating tiered strappy dress with an easy skirt, while dads and boys wear the matching collared short-sleeve shirt in the same floral story.</p>",
    "",
    ...
    li("Garden floral palette", "Red and pink flowers, yellow accents, green leaves, and a cream ground create a fresh matching look that photographs beautifully."),
...
METAFIELDS_JSON="$(jq -nc \
  ...
  [
      ...
      {ownerId: $pid, namespace: "custom", key: "pattern", type: "single_line_text_field", value: "Garden Floral"},
      ...
      {ownerId: $pid, namespace: "mm-google-shopping", key: "custom_label_1", type: "single_line_text_field", value: "Garden Floral"},
...
for image_path in "${MEDIA_FILES[@]}"; do
  ...
  case "$image_name" in
    01-*)
      ...
      ;;
    *)
      alt_text="Garden Floral matching floral dress and short-sleeve shirt."
      ;;
  esac
...
for recap in derived["recap"]:
    ...
    put("Google Shopping / Custom Label 1", "Garden Floral")
    ...
    put("Pattern (product.metafields.custom.pattern)", "Garden Floral")
    ...
    put("Color (product.metafields.shopify.color-pattern)", "Cream, Red, Floral, Multicolor")
...
lines.append("| SHORTCODE | auto → `GFLR` |")
lines.append("| COLOR_TOKEN | auto → `CREAM` |")
lines.append("| FORCE_SPEC_PRICES | true |")
lines.append("")
lines.append("## Vendor fetch status")
lines.append("The direct 1688 page was captcha-blocked during this run, so the attached vendor fit chart and supplied family photos were used as the authoritative source of truth. The supplied size source is fit-reference-only: it publishes suitable height labels `80-150`, mother labels `S-2XL`, father labels `S-4XL`, and an infant table that was excluded because this request only asked for Dress and Shirt designs; it does not publish garment chest, hip, waist, shoulder, or length measurements for the requested shirt-and-dress design. To keep the listing honest and rerunnable, the garment measurements were backfilled from the store's live `citrus-bloom-family-matching-set` grading because it is the closest mixed dress-and-shirt family-set structure currently live on the store. The supplied photos were then used to keep the copy honest to this product's cream-ground red floral tiered strappy dress and collared short-sleeve shirt presentation rather than inheriting any neighbor-specific design claims. Neighbor pricing and size metaobject GIDs were anchored to `citrus-bloom-family-matching-set`. Styling-only white shorts, sandals, handbags, hats, sunglasses, baskets, and other accessories were excluded because this request only asked for the shirt and dress designs. The Shopify taxonomy stays `Outfit Sets` for honest standard-category attributes.")
lines.append("")
...
lines.append("- The dress and shirt measurement ladders were backfilled from the live `citrus-bloom-family-matching-set`; no unsupported mother 3XL/4XL rows were added because the attached mother table stops at 2XL.")
