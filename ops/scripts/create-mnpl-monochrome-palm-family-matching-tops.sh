...
TITLE = "Monochrome Palm Family Matching Tops — Button-Up Shirt"
SEO_TITLE = "Monochrome Palm Family Shirts | Dress Like Mommy"
PRINT_NAME = "Monochrome Palm"
SHORTCODE = "MNPL"
COLOR_TOKEN = "MONOPALM"
COLOR_NAME = "Monochrome Palm"
...
SCRIPT_PATH = ROOT / "ops/scripts/create-mnpl-monochrome-palm-family-matching-tops.sh"
SOURCE_SIZE_CHART = ROOT / "ops/listings/source-size-chart-family-matching-casual-shirts.png"
PRODUCT_IMAGE = UPLOAD_DIR / "01-monochrome-palm-shirt.png"
...
SEO_DESCRIPTION = (
    "Monochrome Palm shirts for mom, dad, girls & boys. Fabric composition "
...
COLOR_PATTERN_GIDS = [
    "gid://shopify/Metaobject/69943132257",  # Black
...
def build_body() -> str:
    ...
    return "\n\n".join([
        ...
        "<li><strong>Print reference:</strong> Monochrome Palm places black palm-tree silhouettes across a clean white ground.</li>",
        "<li><strong>Design details:</strong> Collared neckline, button-up front, short sleeves, and one shared palm print across child and adult sizes. The hanger, necklace, and white undershirt shown are styling or display pieces only and are not included.</li>",
        ...
        *rendered,
        "</tbody></table>",
        "<p>Monochrome Palm brings a crisp tropical mood to family matching with a white base and repeating black palm-tree silhouettes. The collared button-up shape keeps the high-contrast print easy to coordinate for vacation days, celebrations, and family portraits.</p>",
        ...
        "<li><strong>Distinct tropical print:</strong> Black palm-tree silhouettes and a white ground create a photo-ready monochrome palette.</li>",
        ...
        "</ul>",
        "<p>Choose the child and adult sizes you need to create a coordinated Monochrome Palm look for the next sunny family memory.</p>",
    ])
...
def metafields(product_id: str) -> list[dict]:
    ...
    return [
        ...
        {"ownerId": product_id, "namespace": "custom", "key": "style", "type": "single_line_text_field", "value": "Palm Print Button-Up Shirt"},
...
def validate_preflight(body: str, variants: list[dict]) -> None:
    ...
    for row, variant in zip(SIZE_CHART, variants):
        ...
        if not variant["inventoryItem"]["sku"].startswith("DLM-MNPL-"):
            errors.append("SKU prefix is not DLM-MNPL")
...
def upload_media(product_id: str) -> None:
    ...
    path = PRODUCT_IMAGE
    alt = "White short-sleeve button-up shirt with black palm-tree silhouettes."
...
def write_csv(body: str, variants: list[dict]) -> None:
    ...
    for i, (row, variant) in enumerate(zip(SIZE_CHART, variants), start=1):
        ...
        values.update({
            ...
            "Style (product.metafields.custom.style)": "Palm Print Button-Up Shirt" if i == 1 else "",
            ...
            "Color (product.metafields.shopify.color-pattern)": "Black, White" if i == 1 else "",
...
if __name__ == "__main__":
    main()
PY
/usr/bin/python3 "$ROOT/ops/scripts/finalize_shopify_listing_localization.py" --handles "monochrome-palm-family-matching-tops"
