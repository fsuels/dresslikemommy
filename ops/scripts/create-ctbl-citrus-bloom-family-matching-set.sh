...
TITLE="Citrus Bloom Family Matching Set — Dress & Shirt"
SEO_TITLE="Citrus Bloom Family Set | Dress Like Mommy"
SEO_DESCRIPTION="Lightweight woven family matching set with floral dresses and collared shirts for moms, dads, girls & boys. Sizes 1-2Y–10Y, Mother S–2XL, Father S–4XL."
PRINT_NAME="Citrus Bloom"
SHORTCODE="CTBL"
COLOR_TOKEN="BLOOM"
COLOR_NAME="Citrus Bloom"
...
SCRIPT_PATH="${ROOT}/ops/scripts/create-ctbl-citrus-bloom-family-matching-set.sh"
...
[
  ...
  {"picker_label":"Mother XL","gid":"gid://shopify/Metaobject/129975287905","catalog_label":"XL","source_handle":"pink-horizon-family-matching-set"},
  {"picker_label":"Mother 2XL","gid":"gid://shopify/Metaobject/129975156833","catalog_label":"2XL","source_handle":"pink-horizon-family-matching-set"},
  {"picker_label":"Father S","gid":"gid://shopify/Metaobject/129975255137","catalog_label":"S","source_handle":"pink-horizon-family-matching-set"},
  ...
  {"picker_label":"Father 4XL","gid":"gid://shopify/Metaobject/139840716897","catalog_label":"4XL","source_handle":"pink-horizon-family-matching-set"}
]
JSON
...
body_html = "\n".join([
    ...
    li("Family story", "A coordinated four-role matching look for moms, dads, girls, and boys, made for lakeside photos, vacations, brunches, and easy summer outings."),
    li("Print", f"\"{print_name}\" mixes painterly yellow citrus florals, blue petals, and leafy accents over a soft sky-blue ground for a bright, photo-ready palette."),
    li("Design details", "Girls and moms wear the coordinating shoulder-strap dress with a gathered silhouette, while boys and dads get the matching collared button-front shirt with short sleeves. The white shorts shown in the lifestyle photos are styling only and not included."),
    ...
    *shirt_rows,
    ...
    "</table>",
    "",
    "<p>Citrus Bloom brings a sunny, painterly mood to family matching. Moms and girls get the coordinating floral dress with an easy gathered silhouette, while dads and boys wear the matching collared short-sleeve shirt in the same blue-and-yellow print. The whole look feels bright, cheerful, and naturally photo-ready.</p>",
    "",
    "<p>The dresses keep the girls' and moms' side airy and flowing for warm-weather plans, while the shirts give the boys' and dads' side a neat, relaxed finish with a clean collar and button front. Pack it for family portraits, garden dinners, vacations, or any sunny-day plan where you want every role to feel coordinated and comfortable.</p>",
    "",
    "<h3>Key Features:</h3>",
    "<ul>",
    li("Four-role coordination", "One listing covers girl dress, mother dress, boy shirt, and father shirt sizes."),
    li("Citrus floral palette", "Sky-blue, yellow, and leafy accents keep the family look lively, bright, and easy to style."),
    li("Dress ease", "The shoulder-strap dress gives girls and moms an airy, easy-moving silhouette for warm-weather plans."),
    li("Shirt polish", "Collared short-sleeve shirts add a neat finish for boys and dads without feeling stiff."),
    li("White shorts not included", "The white shorts shown on the dad model are styling only and were intentionally excluded from this listing."),
    "</ul>",
    "",
    "<p>Choose each role and size you need, and build a family matching look that feels bright, polished, and ready for the next sunny memory.</p>",
])
...
COLOR_PATTERN_GIDS_JSON='["gid://shopify/Metaobject/69622104161","gid://shopify/Metaobject/69639766113","gid://shopify/Metaobject/129971519585","gid://shopify/Metaobject/130231140449"]'
...
METAFIELDS_JSON="$(jq -nc \
  ...
  [
      ...
      {ownerId: $pid, namespace: "custom", key: "pattern", type: "single_line_text_field", value: "Citrus Bloom Floral"},
      ...
      {ownerId: $pid, namespace: "mm-google-shopping", key: "custom_label_1", type: "single_line_text_field", value: "Citrus Bloom"},
...
for image_path in "${MEDIA_FILES[@]}"; do
  ...
  case "$image_name" in
    01-*)
      alt_text="Family in Citrus Bloom matching dress and floral short-sleeve shirt by the lakeside."
      ;;
    *)
      alt_text="Citrus Bloom family matching dress and shirt set for moms, dads, girls, and boys."
      ;;
  esac
...
for recap in derived["recap"]:
    ...
    put("Google Shopping / Custom Label 1", "Citrus Bloom")
    ...
    put("Pattern (product.metafields.custom.pattern)", "Citrus Bloom Floral")
    ...
    put("Color (product.metafields.shopify.color-pattern)", "Yellow, Blue, Floral, Multicolor")
...
lines.append("| EXCLUDE_ITEMS | infant romper table and white shorts styling-only imagery excluded because the request only asked for Shirt + Dress |")
lines.append("| SHORTCODE | auto → `CTBL` |")
lines.append("| COLOR_TOKEN | auto → `BLOOM` |")
...
lines.append("- The attached vendor chart is a fit-reference-only table, so every garment measurement column (`chest_cm`, `hip_cm`, `waist_cm`, `shoulder_cm`, `length_cm`, `skirt_cm`) was backfilled from the closest live dress/shirt grading curves already published on the store rather than fabricated from scratch.")
...
lines.append("- Dress measurements were anchored to the live `pink-horizon-family-matching-set` dress ladder for `90-150` and `Mother S-2XL`; the `80` row continues that same grade one step down because the vendor fit chart includes `80` but the live anchor starts at `90`.")
lines.append("- Shirt measurements were anchored to the live `summer-sky-stripe-family-matching-set` collared short-sleeve shirt ladder for `80-150` and `Father S-3XL`; the `Father 4XL` row continues that same adult grade one step up because the vendor fit chart includes `4XL` while the live collared-shirt anchor stops at `3XL`.")
lines.append("- The infant romper reference table on the supplied size chart was intentionally excluded because it describes a separate baby garment family outside this `Dress` + `Shirt` listing request.")
...
lines.append("- Women S / M / L / XL / 2XL → Mother S / M / L / XL / 2XL")
...
lines.append("- The infant romper reference table on the supplied size chart was excluded because it belongs to a different baby garment family than the requested shirt-and-dress listing.")
lines.append("- The white shorts shown on the dad model in the supplied photos are styling only and were excluded because the request explicitly limited the listing to the shirt and dress designs.")
lines.append("")
...
lines.append("- Garment grading anchors: `pink-horizon-family-matching-set` for the dress ladder and `summer-sky-stripe-family-matching-set` for the collared-shirt ladder")
