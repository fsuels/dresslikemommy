...
HANDLE = "bright-paint-splash-family-matching-tops"
TITLE = "Bright Paint Splash Family Matching Tops — Button-Up Shirt"
SEO_TITLE = "Bright Paint Splash Family Shirts | Dress Like Mommy"
PRINT_NAME = "Bright Paint Splash"
SHORTCODE = "BPSH"
COLOR_TOKEN = "BRISPT"
COLOR_NAME = "Bright Paint Splash"
...
SCRIPT_PATH = ROOT / "ops/scripts/create-bpsh-bright-paint-splash-family-matching-tops.sh"
SOURCE_SIZE_CHART = ROOT / "ops/listings/source-size-chart-family-matching-casual-shirts.png"
PRODUCT_IMAGE = UPLOAD_DIR / "01-bright-paint-splash-shirt.png"
...
SIZE_CHART = [
    ...
    {"audience":"child","role":"Child Shirt","garment":"Shirt","vendor_label":"160","picker_label":"Child 12 Years","sku_suffix":"KID12Y","age":"12","weight":"32.5-40 kg","height":"145-150 cm","chest_cm":"—","hip_cm":"—","waist_cm":"—","length_cm":"—","sleeve_cm":"—","pant_cm":"—"},
...
SEO_DESCRIPTION = (
    "Bright Paint Splash shirts for mom, dad, girls & boys. Fabric composition "
    f"is unspecified; sizes {SEO_SIZE_PHRASE}."
)
SIZE_RANGE_COPY = (
...
def build_body() -> str:
    ...
    return "\n\n".join([
        "<ul>",
        "<li><strong>Fabric:</strong> Exact fiber composition and fabric construction were not available in the supplied evidence.</li>",
        ...
        "<li><strong>Print reference:</strong> Bright Paint Splash layers vivid blue, teal, orange, red, and yellow paint marks across a white ground.</li>",
        "<li><strong>Design details:</strong> Collared neckline, button-up front, short sleeves, and a blue-pattern chest pocket across child and adult sizes. The hanger and white undershirt shown are styling or display pieces only and are not included.</li>",
        ...
        "<table id=\"size-chart\">",
        "<thead><tr>",
        *[f"<th>{header}</th>" for header in TABLE_HEADERS],
        "</tr></thead>",
        "<tbody>",
        *rendered,
        "</tbody></table>",
        "<p>Bright Paint Splash brings energetic color to family matching with a white base, bold painterly marks, and a contrasting blue-pattern chest pocket. The collared button-up shape makes the vivid print an easy focal point for vacation days, celebrations, creative outings, and family portraits.</p>",
        ...
        "<li><strong>Expressive paint print:</strong> Vivid blue, teal, orange, red, and yellow splashes create a playful photo-ready palette.</li>",
        "<li><strong>Easy warm-weather styling:</strong> The short-sleeve button-up shape pairs easily with simple family outfits.</li>",
        ...
        "</ul>",
        "<p>Choose the child and adult sizes you need to create a coordinated Bright Paint Splash look for the next colorful family memory.</p>",
    ])
...
def metafields(product_id: str) -> list[dict]:
    ...
    return [
        ...
        {"ownerId": product_id, "namespace": "custom", "key": "style", "type": "single_line_text_field", "value": "Paint Splash Button-Up Shirt"},
...
def validate_preflight(body: str, variants: list[dict]) -> None:
    ...
    rendered_headers = [
        ...
        for value in re.findall(r"<th>(.*?)</th>", tables[0] if tables else "", re.S)
    ]
    if rendered_headers != TABLE_HEADERS:
        errors.append("body size-table headers do not match the canonical order")
    ...
    for row, variant in zip(SIZE_CHART, variants):
        ...
        if not variant["inventoryItem"]["sku"].startswith("DLM-BPSH-"):
            errors.append("SKU prefix is not DLM-BPSH")
    ...
    if VENDOR_URL:
        errors.append("external source URL must remain empty")
            ...
            json.dumps(
                {
                    "raw_detail_text": (
                        ...
                        "one purchasable shirt only; styling layer and hanger are not included"
                    )
                }
            ),
            encoding="utf-8",
...
def upload_media(product_id: str) -> None:
    ...
    path = PRODUCT_IMAGE
    alt = "White short-sleeve button-up shirt with vivid multicolor paint splashes and a blue-pattern chest pocket."
...
def write_csv(body: str, variants: list[dict]) -> None:
    ...
    for i, (row, variant) in enumerate(zip(SIZE_CHART, variants), start=1):
        ...
        values.update({
            ...
            "Style (product.metafields.custom.style)": "Paint Splash Button-Up Shirt" if i == 1 else "",
            ...
            "Color (product.metafields.shopify.color-pattern)": "Blue, White, Multicolor" if i == 1 else "",
...
def write_listing(product_id: str, verify: dict, variants: list[dict], price_rows: list[dict]) -> None:
    ...
    skipped = [
        ...
        ("shopify.color-pattern Teal/Orange/Red/Yellow", "The visible colors remain in copy and tags; only verified Blue, White, and Multicolor catalog references are written."),
    ...
    lines = [
        ...
        "| DESIGNS_TO_LIST | one Bright Paint Splash short-sleeve button-up shirt |",
        "| FORCE_SPEC_PRICES | true |",
        ...
        *recap, "",
        "## Derivations",
        "- The supplied product image supports one unisex white collared, short-sleeve button-up shirt with vivid blue, teal, orange, red, and yellow paint splashes plus a blue-pattern chest pocket. The hanger and white undershirt are not included.",
...
if __name__ == "__main__":
    main()
PY
/usr/bin/python3 "$ROOT/ops/scripts/finalize_shopify_listing_localization.py" --handles "bright-paint-splash-family-matching-tops"
