...
TITLE = "Coastal Banana Leaf Family Matching Tops — Button-Up Shirt"
SEO_TITLE = "Coastal Banana Leaf Family Shirts | Dress Like Mommy"
PRINT_NAME = "Coastal Banana Leaf"
SHORTCODE = "CBLF"
COLOR_TOKEN = "COASTLF"
COLOR_NAME = "Coastal Banana Leaf"
...
SCRIPT_PATH = ROOT / "ops/scripts/create-cblf-coastal-banana-leaf-family-matching-tops.sh"
SOURCE_SIZE_CHART = ROOT / "ops/listings/source-size-chart-family-matching-casual-shirts.png"
PRODUCT_IMAGE = UPLOAD_DIR / "01-coastal-banana-leaf-shirt.png"
...
SEO_DESCRIPTION = (
    "Coastal Banana Leaf shirts for mom, dad, girls & boys in lightweight "
...
def build_body() -> str:
    ...
    return "\n\n".join([
        ...
        "<li><strong>Print reference:</strong> Coastal Banana Leaf layers vivid blue and green banana and tropical leaves with slim orange botanical accents over a white ground.</li>",
        "<li><strong>Design details:</strong> Collared neckline, button-up front, short sleeves, and one shared print across child and adult sizes. The white undershirt and hanger shown are styling or display pieces only and are not included.</li>",
        ...
        *rendered,
        "</tbody></table>",
        "<p>Coastal Banana Leaf brings a breezy tropical palette to family matching with a crisp white base, overlapping blue and green leaf shapes, and slim orange botanical accents. The open-collar button-up silhouette makes the print easy to style over a plain undershirt or wear on its own for coordinated vacation days and family portraits.</p>",
        ...
        "<li><strong>Distinct tropical print:</strong> Blue and green banana and tropical leaves, a white ground, and slim orange botanical accents create a photo-ready family palette.</li>",
        ...
        "</ul>",
        "<p>Choose the child and adult sizes you need to create a coordinated Coastal Banana Leaf look for the next sunny family memory.</p>",
    ])
...
def validate_preflight(body: str, variants: list[dict]) -> None:
    ...
    for row, variant in zip(SIZE_CHART, variants):
        ...
        if not variant["inventoryItem"]["sku"].startswith("DLM-CBLF-"):
            errors.append("SKU prefix is not DLM-CBLF")
...
def upload_media(product_id: str) -> None:
    ...
    path = PRODUCT_IMAGE
    alt = "Coastal Banana Leaf short-sleeve button-up shirt with blue-green tropical leaves and slim orange accents on white."
...
def write_csv(body: str, variants: list[dict]) -> None:
    ...
    for i, (row, variant) in enumerate(zip(SIZE_CHART, variants), start=1):
        ...
        values.update({
            ...
            "Style (product.metafields.custom.style)": "Tropical Button-Up Shirt" if i == 1 else "",
...
if __name__ == "__main__":
    main()
PY
/usr/bin/python3 "$ROOT/ops/scripts/finalize_shopify_listing_localization.py" --handles "coastal-banana-leaf-family-matching-tops"
