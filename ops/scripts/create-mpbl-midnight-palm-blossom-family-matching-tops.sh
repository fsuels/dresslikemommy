...
HANDLE = "midnight-palm-blossom-family-matching-tops"
TITLE = "Midnight Palm Blossom Family Matching Tops — Button-Up Shirt"
SEO_TITLE = "Midnight Palm Family Shirts | Dress Like Mommy"
PRINT_NAME = "Midnight Palm Blossom"
SHORTCODE = "MPBL"
COLOR_TOKEN = "MIDNIGHT"
COLOR_NAME = "Midnight Palm Blossom"
...
SCRIPT_PATH = ROOT / "ops/scripts/create-mpbl-midnight-palm-blossom-family-matching-tops.sh"
SOURCE_SIZE_CHART = ROOT / "ops/listings/source-size-chart-family-matching-casual-shirts.png"
PRODUCT_IMAGE = UPLOAD_DIR / "01-midnight-palm-blossom-shirt.png"
...
SEO_DESCRIPTION = (
    "Midnight Palm Blossom shirts for mom, dad, girls & boys in lightweight "
...
def build_body() -> str:
    ...
    return "\n\n".join([
        ...
        "<li><strong>Print reference:</strong> Midnight Palm Blossom layers green and pale palm fronds with pink and coral tropical blossoms over a dark navy ground.</li>",
        ...
        *rendered,
        "</tbody></table>",
        "<p>Midnight Palm Blossom brings a polished tropical mood to family matching with a deep navy base, layered palm shapes, and bright pink-coral blossoms. The open-collar button-up silhouette makes the print easy to style over a plain undershirt or wear on its own for coordinated vacation days and family portraits.</p>",
        ...
        "<li><strong>Distinct tropical print:</strong> Dark navy, green and pale palm fronds, and pink-coral blossoms create a photo-ready family palette.</li>",
        ...
        "</ul>",
        "<p>Choose the child and adult sizes you need to create a coordinated Midnight Palm Blossom look for the next sunny family memory.</p>",
    ])
...
def validate_preflight(body: str, variants: list[dict]) -> None:
    ...
    for row, variant in zip(SIZE_CHART, variants):
        ...
        if not variant["inventoryItem"]["sku"].startswith("DLM-MPBL-"):
            errors.append("SKU prefix is not DLM-MPBL")
...
def upload_media(product_id: str) -> None:
    ...
    path = PRODUCT_IMAGE
    alt = "Midnight Palm Blossom short-sleeve button-up shirt with dark navy palms and pink-coral blossoms."
...
def write_csv(body: str, variants: list[dict]) -> None:
    ...
    for i, (row, variant) in enumerate(zip(SIZE_CHART, variants), start=1):
        ...
        values.update({
            ...
            "Color (product.metafields.shopify.color-pattern)": "Blue, Pink, Floral, Multicolor" if i == 1 else "",
...
def write_listing(product_id: str, verify: dict, variants: list[dict], price_rows: list[dict]) -> None:
    ...
    skipped = [
        ...
        ("shopify.color-pattern Green/Coral", "The visible colors remain in copy and tags; only verified Blue, Pink, Floral, and Multicolor catalog references are written."),
    ...
    lines = [
        ...
        "| DESIGNS_TO_LIST | one Midnight Palm Blossom short-sleeve button-up shirt |",
        "| FORCE_SPEC_PRICES | true |",
        ...
        *recap, "",
        "## Derivations",
        "- The supplied product image supports one unisex collared, short-sleeve button-up shirt in a dark navy, palm-frond, and pink-coral blossom print. The white undershirt and hanger are not included.",
...
if __name__ == "__main__":
    main()
PY
/usr/bin/python3 "$ROOT/ops/scripts/finalize_shopify_listing_localization.py" --handles "midnight-palm-blossom-family-matching-tops"
