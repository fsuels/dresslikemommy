#!/usr/bin/env python3
"""Install or manage the local macOS launchd job for Shopify import autofill."""
...
STATE_PATH = DEFAULT_CONFIG_DIR / "shopify-import-autofill-state.json"
WORKER_PATH = REPO_ROOT / "ops" / "scripts" / "poll_shopify_import_autofill.py"
...
def build_program_arguments(parsed: argparse.Namespace) -> list[str]:
    args = [
        ...
        "--jsonl-log",
        str((LOG_DIR / "shopify-import-autofill.jsonl").expanduser()),
        "--min-age-seconds",
    ...
    if parsed.execute:
        args.append("--execute")
    if parsed.vendor_contains:
        args.extend(["--vendor-contains", parsed.vendor_contains])
    if parsed.required_tag:
        args.extend(["--required-tag", parsed.required_tag])
    return args


def plist_payload(parsed: argparse.Namespace) -> dict[str, Any]:
    stdout_path = LOG_DIR / "shopify-import-autofill.stdout.log"
    stderr_path = LOG_DIR / "shopify-import-autofill.stderr.log"
    return {
        "Label": LABEL,
...
def main() -> None:
    ...
    parser.add_argument("--min-age-seconds", type=int, default=180, help="Wait this long after creation before processing.")
    ...
    parser.add_argument("--max-products-per-run", type=int, default=10, help="Maximum products handled per run.")
