...
HANDLE = "sky-daisy-doodle-family-matching-tops"
TITLE = "Sky Daisy Doodle Family Matching Tops — Button-Up Shirt"
SEO_TITLE = "Sky Daisy Family Shirts | Dress Like Mommy"
PRINT_NAME = "Sky Daisy Doodle"
SHORTCODE = "SDSY"
COLOR_TOKEN = "SKYDSY"
COLOR_NAME = "Sky Daisy Doodle"
...
SCRIPT_PATH = ROOT / "ops/scripts/create-sdsy-sky-daisy-doodle-family-matching-tops.sh"
SOURCE_SIZE_CHART = ROOT / "ops/listings/source-size-chart-family-matching-casual-shirts.png"
PRODUCT_IMAGE = UPLOAD_DIR / "01-sky-daisy-doodle-shirt.png"
...
SEO_DESCRIPTION = (
    "Sky Daisy Doodle shirts for mom, dad, girls & boys in lightweight "
...
COLOR_PATTERN_GIDS = [
    ...
    "gid://shopify/Metaobject/69622104161",  # Yellow
...
def build_body() -> str:
    ...
    return "\n\n".join([
        ...
        "<li><strong>Print reference:</strong> Sky Daisy Doodle pairs oversized white daisies and warm yellow centers with loose blue doodle and letter-like marks over a pale sky-blue ground.</li>",
        "<li><strong>Design details:</strong> Collared neckline, button-up front, short sleeves, a chest patch pocket, and one shared print across child and adult sizes. The white undershirt and hanger shown are styling or display pieces only and are not included.</li>",
        ...
        *rendered,
        "</tbody></table>",
        "<p>Sky Daisy Doodle brings a cheerful graphic mood to family matching with a pale sky-blue base, oversized white daisies, warm yellow centers, and loose blue doodle marks. The open-collar button-up silhouette makes the print easy to style over a plain undershirt or wear on its own for coordinated vacation days and family portraits.</p>",
        ...
        "<li><strong>Distinct daisy print:</strong> Pale sky blue, oversized white daisies, yellow centers, and blue doodle marks create a photo-ready family palette.</li>",
        "<li><strong>Easy warm-weather layer:</strong> The relaxed button-up shape works over a simple undershirt or as a standalone top.</li>",
        ...
        "</ul>",
        "<p>Choose the child and adult sizes you need to create a coordinated Sky Daisy Doodle look for the next sunny family memory.</p>",
    ])
...
def metafields(product_id: str) -> list[dict]:
    ...
    return [
        ...
        {"ownerId": product_id, "namespace": "custom", "key": "style", "type": "single_line_text_field", "value": "Daisy Print Button-Up Shirt"},
...
def validate_preflight(body: str, variants: list[dict]) -> None:
    ...
    for row, variant in zip(SIZE_CHART, variants):
        ...
        if not variant["inventoryItem"]["sku"].startswith("DLM-SDSY-"):
            errors.append("SKU prefix is not DLM-SDSY")
...
def upload_media(product_id: str) -> None:
    ...
    path = PRODUCT_IMAGE
    alt = "Sky Daisy Doodle short-sleeve button-up shirt in pale blue with white daisies, yellow centers, and blue doodle marks."
...
def write_csv(body: str, variants: list[dict]) -> None:
    ...
    for i, (row, variant) in enumerate(zip(SIZE_CHART, variants), start=1):
        ...
        values.update({
            ...
            "Style (product.metafields.custom.style)": "Daisy Print Button-Up Shirt" if i == 1 else "",
            ...
            "Color (product.metafields.shopify.color-pattern)": "Blue, White, Yellow, Floral, Multicolor" if i == 1 else "",
...
def write_listing(product_id: str, verify: dict, variants: list[dict], price_rows: list[dict]) -> None:
    ...
    skipped = [
        ...
        ("shopify.sleeve-length-type", "Short sleeves are visible, but no owner-subtype-safe catalog value was verified for this Dress Shirts taxonomy run."),
        ("shopify.neckline", "The garment has a shirt collar, and no owner-subtype-safe neckline value was verified for this Dress Shirts taxonomy run."),
        ...
        ("shopify.color-pattern Pale Sky Blue", "The pale sky-blue shade remains in copy and tags; verified Blue, White, Yellow, Floral, and Multicolor catalog references are written without inventing a separate pale-blue match."),
    ...
    lines = [
        ...
        "| DESIGNS_TO_LIST | one Sky Daisy Doodle short-sleeve button-up shirt |",
        "| FORCE_SPEC_PRICES | true |",
        ...
        *recap, "",
        "## Derivations",
        "- The supplied product image supports one unisex collared, short-sleeve button-up shirt with a chest patch pocket, pale sky-blue ground, oversized white daisies, yellow centers, and loose blue doodle or letter-like marks. The white undershirt and hanger are not included.",
...
if __name__ == "__main__":
    main()
PY
/usr/bin/python3 "$ROOT/ops/scripts/finalize_shopify_listing_localization.py" --handles "sky-daisy-doodle-family-matching-tops"
