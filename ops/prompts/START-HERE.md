...
Localized size-chart default: after every product create/update with a body size table, run the Shopify product translation flow for the handle, then run `ops/scripts/repair_localized_product_size_charts.py --handles <handle> --execute` and a dry readback. Also run `ops/scripts/audit_localized_size_chart_variant_mapping.py --handles <handle> --fail-on-unmatched` so every available variant can map to a visible row/card in every published locale. Do not call the listing complete until every published non-primary locale either falls back to the source body or has a `body_html` translation with the size-chart table, and the variant-row audit has `unmatched_variant_locale_count=0`. This protects localized PDPs where Shopify serves native translated `body_html`.
