...
Full-localization default: every generated `ops/scripts/create-<shortcode>-<slug>.sh` runner must end by calling `/usr/bin/python3 "$ROOT/ops/scripts/finalize_shopify_listing_localization.py" --handles "<handle>"`. This synchronous closeout forces an immediate full-product translation refresh with `--min-age-seconds 0`, audits every published non-primary locale before and after deterministic size-chart repair, and fails the listing runner when any translation, localized size-chart, or variant-row mapping gate fails. The five-minute LaunchAgent remains a second safety net for products created outside the normal runner. A source-language body with translated labels is a failure, not a localized fallback. Do not call the listing complete without a `status=passed` closeout report at `ops/listings/<handle>-localization-closeout.json`.
