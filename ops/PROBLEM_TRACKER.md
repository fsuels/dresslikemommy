...
| `PROB-2026-05-15-PINTEREST-FEED-VARIANT-DUPLICATION` | `P1` | `DIAGNOSED_ALL_MARKETS_ALL_CATEGORIES_APPROVAL_PACKETS_READY__GUARDRAIL_WIRED__NO_LIVE_WRITE_YET` | Claude current session / next paid-growth + Shopify channel operator | Shopify -> Pinterest sales-channel product feed for **every** active Shopify Market (`us`, `canada`, `united-kingdom`, `eu`, `australia`, `international`) and **every** product category (Family Matching, Dresses, Couples, Sweaters, plus any future types); Pinterest advertiser `549756244483`; Merchant Center mirror feed | Owner reviews `dresslikemommy-growth-2026/02_AUDIT_PACKETS/2026-05-15-pinterest-feed-grouping-all-markets-fix/CROSS_MARKET_VARIANT_DUPLICATION_DIAGNOSIS.md` and either pastes `MASTER_ALL_MARKETS_APPROVAL_PHRASE.md` to fix every market in one shot, or pastes a single per-market approval phrase from `per_market_packets/` (US first, then CA, GB, EU, AU, International). After each market's 24h re-sync, capture after-state readback per `per_market_packets/<market>` checklist, then create `FIX_LANDED_FRESHNESS_MARKER.txt` in the packet folder to flip the automated guardrail to strict. If the Shopify Pinterest channel UI does not expose a parent-grouping toggle for a market, fall back to Path B with `ops/scripts/generate_pinterest_feed_grouped.py --market <handle>` and upload as a separate Pinterest catalog source under a fresh exact approval. Do not touch Pinterest tag/CAPI/budget/bid/status/campaign/audience/billing, Shopify product data, or other sales channels | Every active Shopify Market emits Pinterest catalog rows where same-parent variants share `item_group_id` and `image_link` is the parent product featured image; per-market collapse from `~20x` variant inflation to one row per parent (or grouped rows); the automated guard `ops/scripts/check_pinterest_feed_grouping.py` runs in strict mode under `ops/scripts/check_continuity_integrity.py --strict` and returns PASS for every market; freshness marker file exists with per-market after-state readback summary; AGENTS.md and CLAUDE.md non-negotiable rule preserved byte-identical | `dresslikemommy-growth-2026/02_AUDIT_PACKETS/2026-05-15-pinterest-feed-grouping-all-markets-fix/CROSS_MARKET_VARIANT_DUPLICATION_DIAGNOSIS.md`; `dresslikemommy-growth-2026/02_AUDIT_PACKETS/2026-05-15-pinterest-feed-grouping-all-markets-fix/MASTER_ALL_MARKETS_APPROVAL_PHRASE.md`; `dresslikemommy-growth-2026/02_AUDIT_PACKETS/2026-05-15-pinterest-feed-grouping-all-markets-fix/per_market_packets/*.md` (6 markets); `ops/scripts/check_pinterest_feed_grouping.py`; `ops/scripts/generate_pinterest_feed_grouped.py`; `ops/scripts/check_continuity_integrity.py` (guardrail wiring); `AGENTS.md` + `CLAUDE.md` Non-Negotiables update; prior packet `dresslikemommy-growth-2026/02_AUDIT_PACKETS/2026-05-15-pinterest-variant-duplication-feed-fix/` retained as Dresses-only diagnosis predecessor |
...
- Priority: `P1`
- Status: `DIAGNOSED_ALL_MARKETS_ALL_CATEGORIES_APPROVAL_PACKETS_READY__GUARD_FAIL_EXPECTED`
- Owner/session: Codex automation current session, 2026-05-15 09:45 EDT / next paid-growth + Shopify channel operator.
...
- Apply the Shopify Pinterest channel grouping toggle if exposed. If not exposed, use `ops/scripts/generate_pinterest_feed_grouped.py --market <handle>` as Path B only after a separate exact approval.
- After 24h Pinterest re-sync, capture per-market catalog counts, group counts, item_group_id proof, and new-disapproval readback; only then replace the freshness marker body with the required attest line and run strict continuity.
