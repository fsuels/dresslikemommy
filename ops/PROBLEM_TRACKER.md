...
Status: `CUSTOM_PIXEL_OWNER_INSTALL_READY__CONTROLLED_PURCHASE_STILL_OWNER_ONLY`
...
Current next action:
- Owner installs the local Custom Pixels using `docs/tracking-setup.md`: create GA4 API secret, create native Google Ads conversion action ID/label, keep real values out of the repo, replace placeholders only in Shopify's editor or a non-repo temporary copy, connect both Shopify Custom Pixels, and verify DebugView / Tag Assistant / first real purchase.
