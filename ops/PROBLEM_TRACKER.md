...
Current next action:
- Use logged-in GA4 UI Explore/report export for property `330266838`, or refresh read-only GA4 Data/Admin API scopes, to match the sanitized Shopify non-USD order candidates to actual `purchase` event currency/value/transaction evidence. If neither read-only path exposes the event-level fields and the owner wants this gate closed now, request exact action-time approval for a controlled non-US test purchase/refund/cancel procedure.
