...
| `PROB-2026-05-14-US-SHOPPING-QUERY-TITLE-FIT` | `P1` | `DIAGNOSIS_PACKET_READY__AUTH_EXPORT_REQUIRED` | Codex automation current session / next Google Ads + Merchant operator | US Standard Shopping campaign `23802638621`; item-level title/query/product-fit proof | Run authenticated read-only product-item export for campaign `23802638621` with item ID, title, product group/custom label, impressions, clicks, cost, query where available, conversion value, and landing URL; join to `us_shopping_query_title_candidates.csv` before any product/feed/title recommendation | Item-level Shopping export proves titles/product fit either match query demand or identifies exact mismatches for a narrow owner-approved product/feed title repair packet; no negative/product-group/bid/status action from zero-click zero-cost terms | `dresslikemommy-growth-2026/02_AUDIT_PACKETS/2026-05-14-automation-us-shopping-query-title-diagnosis/US_STANDARD_SHOPPING_QUERY_TITLE_DIAGNOSIS.md` |
...
- Priority: `P1`
- Status: `DIAGNOSIS_PACKET_READY__AUTH_EXPORT_REQUIRED`
- Owner/session: Codex automation current session, 2026-05-14 12:43 EDT / next Google Ads + Merchant operator
