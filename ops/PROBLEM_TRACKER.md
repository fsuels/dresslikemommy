...
Status: `PARTIAL_9_APPLIED_REMAINING_BLOCKED_BY_FR_STALE_PREVIEW_BE_THROTTLE_IT_IN_PROGRESS_PREVIEW`
...
Current next action:
- Do not request the same approval again and do not re-upload completed countries. Resume only unresolved split files (`FR`, `BE`, `IT`, `PL`, `CZ`, `RO`, `PT`, `GR`) after Ads upload cooldown/tooling is clean; preview/download/validate/apply/download/validate one country at a time, with campaign RPC readback after each. `FR` needs a fresh completed `88/88 # OK` preview; `BE` needs upload-throttle cooldown; `IT` needs the in-progress `0`-change preview to clear or be superseded by a fresh completed `88/88 # OK` preview before apply.
