...
| `PROB-2026-05-14-CANDIDATE-COLLECTION-LANDING-CLEANLINESS` | `P1` | `ACTIVE_SOLVING_ROUTE_HOLDS_SET` | Codex current session / next landing-CRO or Shopify theme/product operator | GB/CA/AU keyword-universe collection routes for future long-tail Search rows | Keep affected rows held in `ops/marketing/keyword_universe.csv`; repair/reroute/exclude `matching-dresses`, `swimsuits`, `vacation`, and `daddy-and-me` before any live traffic. Shopify product/vendor/metadata edits require fresh explicit approval if needed | Every candidate route used for paid traffic returns `200`, has no supplier/source-domain hits, no URL-like analytics brand values, no stale seasonal mismatch, and has country/shipping/readback proof before upload | `dresslikemommy-growth-2026/02_AUDIT_PACKETS/2026-05-14-automation-paid-landing-post-sanitizer-readback/PAID_LANDING_POST_SANITIZER_AND_COLLECTION_PREFLIGHT.md`; `ops/marketing/keyword_universe.csv` |
...
- Keep `matching-dresses`, `swimsuits`, `vacation`, and `daddy-and-me` keyword rows held until the separate collection-route problem is repaired/rerouted/excluded.
...
- Priority: `P1`
- Status: `ACTIVE_SOLVING_ROUTE_HOLDS_SET`
- Owner/session: Codex automation current session, 2026-05-14 11:19 EDT / next landing-CRO or Shopify theme/product operator
...
- Repair/reroute/exclude blocked routes before any live Search traffic. If repair requires Shopify product/vendor/source metadata edits, request fresh exact approval with before/after source readback.
