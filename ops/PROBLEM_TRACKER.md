...
- Priority: `P1`
- Status: `CPC_DECISION_KIT_READY__DIRTY_ROUTE_CLEANUP_PACKET_READY__AUTH_VALIDATION_PENDING`
- Owner/session: Codex automation current session, 2026-05-14 11:59-15:38 EDT / next Google Ads operator with landing-CRO support
...
- Keep `/collections/swimsuits` and `/collections/matching-dresses` excluded until repaired/rerouted; the current GB/CA/AU swimwear validation rows use the clean `/collections/family-swimsuits` route, and matching-dress GB/CA/AU rows use clean alternate routes. If repair requires Shopify product/vendor/source metadata edits, use `COLLECTION_SOURCE_CLEANUP_APPROVAL_PACKET.md` for fresh exact approval with before/after source readback.
