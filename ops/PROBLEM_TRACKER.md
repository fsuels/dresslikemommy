...
- Priority: `P1`
- Status: `CLICKED_TITLE_CLEANUP_PACKET_READY__OWNER_APPROVAL_REQUIRED`
- Owner/session: Codex automation current session, 2026-05-14 12:43-2026-05-15 06:08 EDT / next Google Ads + Merchant + Shopify title operator
...
Status: `PRIORITY_MARKET_REGION_PREVIEW_READY__LIVE_SCOPE_READBACK_REQUIRED`

Owner/session: Codex current session, 2026-05-15 read-only Merchant issue-export/all-products export plus owner priority-market correction; next Merchant/growth agent owns exact platform readback and scoped capacity cleanup execution.
...
Current next action:
- Execute only after exact platform control-surface readback reconciles to `merchant_capacity_platform_preview_acceptance.csv` and, if using Shopify Markets, `shopify_international_region_prune_preview.csv`: remove only preview-matched Asian/Middle East, African, South American, and non-US-USD publishing scope, preserving USA English/Spanish, separate priority markets, Europe-later groups, and first-pass hold-review regions.
- After cleanup, capture a fresh all-products export and rerun `build_merchant_capacity_execution_guard.py --after-export /path/to/fresh_export.csv`; then enable/export Canada English/French and GB English rows with expected CAD/GBP proof before any Shopping build.

Approval/credential/platform gates:
- Owner direction exists for non-priority country/feed cleanup, but the live mutation still requires exact pre-change preview/readback of the selected platform controls and after-state export. Do not click Save/Apply/Sync/Upload if the UI/API cannot show the candidate groups precisely.
...
- The current Chrome file-upload path is blocked for Pinterest bulk import until Chrome extension file URL access or another upload-capable path is available.
...
- Use the restored authenticated Pinterest Ads Manager tab and refreshed `333` scope only after exact approval to create/expose active-clean product groups from existing feed attributes. Required groups are Mommy & Me `201`, Family Matching `103`, and Pajamas `29`; father-inclusive rows are proof-only until explicitly approved. Keep the held `9` variants excluded unless they are repaired and public-read back clean.
