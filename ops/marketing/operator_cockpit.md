...
1. Run authenticated Google Ads/Keyword Planner validation for `dresslikemommy-growth-2026/02_AUDIT_PACKETS/2026-05-14-automation-36-row-cpc-canonical-url-packet/gb_ca_au_36_clean_route_cpc_validation_rows_canonical_urls.csv` at max `$0.15`, starting with `ops/ACCOUNT_ACCESS_PROTOCOL.md` so the operator claims an existing authenticated Ads tab/session before opening new login pages; export forecast/readback rows, run the parser, then prepare an exact bounded action row only if auction-entry feasibility passes.
...
- The CPC validation decision kit is a no-upload harness. It narrows the account-side work but does not prove CPC feasibility; only authenticated forecast rows classified `PASS_015_CPC_GATE` may become a fresh `GREEN` action row after reviewer and after-state gates. The current controllable Google Ads page is at Google sign-in, so a real authenticated Ads tab/export is still needed.
...
- The US keyword route unblock packet is also no-write prep. It proves replacement route cleanliness for `23` rerouted US rows, but it does not prove active product fit, CPC feasibility, or live Search readiness.
