...
- 2026-05-14 collection-route preflight: `mommy-and-me`, `family-matching`, and `pajamas` passed public source checks for GB/CA/AU. `matching-dresses` and `swimsuits` still leak raw Shopify product JSON supplier vendors; `vacation` returns `404`; `daddy-and-me` has Christmas pattern metadata hits. Affected `keyword_universe.csv` rows are held via `live_action` until fixed/rerouted/excluded.
