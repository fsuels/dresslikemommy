...
Last reconciled: 2026-05-14 11:40 EDT
Data freshness: read-only Google Ads, Pinterest, Merchant, and public/local paid landing reconciliation completed; repo-local GB/CA/AU keyword strategy repair completed; fresh GB/CA/AU Ads monitor and landing gate review completed; post-sanitizer public active-PDP and collection-route preflight/reroute completed; owner `$0.15` CPC correction applied; 105-row local keyword universe/rubric created and route holds refined; automation capability inventory and local Merchant capacity diagnosis completed; command-layer integration audit passed with `0` side-document risks; no external writes.
...
| GB/CA/AU candidate collection routes | n/a | n/a | n/a | n/a | n/a | n/a | `PARTIAL_REROUTE_DONE__31_GREEN_ROWS_CPC_READY__5_SWIM_HELD` | Public source preflight/reroute, `2026-05-14 11:40 EDT`: `mommy-and-me`, `family-matching`, and `pajamas` clean across GB/CA/AU with `200` and `0` leak hits; non-swim matching-dress/vacation/daddy rows rerouted locally; `/collections/swimsuits` still supplier-leak-held |
...
- Local keyword action completed: `ops/marketing/keyword_universe.csv` has `105` scored rows (`60` US, `15` GB, `15` CA, `15` AU; `77` `GREEN`, `20` `YELLOW`, `8` `RED`) plus `keyword_strategy.md` and `keyword_scoring_rubric.md`. This run rerouted GB/CA/AU non-swim rows off dirty/broken collection routes, leaving `31` GB/CA/AU `GREEN` rows clean-route/CPC-validation-ready and `5` swimwear rows held; no live keyword upload occurred.
