...
Last reconciled: 2026-05-14 13:18 EDT
Data freshness: read-only Google Ads, Pinterest, Merchant, and public/local paid landing reconciliation completed; repo-local GB/CA/AU keyword strategy repair completed; fresh GB/CA/AU Ads monitor and landing gate review completed; post-sanitizer public active-PDP and collection-route preflight/reroute completed; swim-route public readback/reroute completed; exact 36-row CPC validation packet prepared; US Standard Shopping query/title diagnosis, public PDP fit preflight, and held-PDP repair/exclusion packet completed; owner `$0.15` CPC correction applied; 105-row local keyword universe/rubric created and route holds refined; automation capability inventory and local Merchant capacity diagnosis completed; command-layer integration audit passed with `0` side-document risks; no external writes.

## Required Daily Rows

| Surface | Spend | Clicks | Impr. | Purchases | Revenue | ROAS | Decision | Freshness |
|---|---:|---:|---:|---:|---:|---:|---|---|
| GB exact Search | `$0.00` | `0` | `0` | `0.00` | `$0.00` | n/a | `HEAD_TERMS_REJECTED__36_CLEAN_ROWS_NEED_015_CPC_VALIDATION` | Google Ads UI/RPC, `2026-05-14T08:10:31-04:00`; search-term filter cleared; keyword/RSA/final URL checks passed; head terms below first page at `$0.65-$0.74`; active PDP final URL source-clean at `2026-05-14T11:18`; route reroute completed at `2026-05-14 12:19`, and clean-route long-tail rows still need `$0.15` CPC validation |
...
| Standard Shopping US | `$0.00` | `0` | `17` | `0.00` | `$0.00` | n/a | `US_SHOPPING_PUBLIC_CLEAN_SCOPE_READY__HELD_PDP_REPAIR_PACKET_READY__READ_ONLY_EXPORT_NEXT` | Google Ads UI, `2026-05-14T05:35`, reporting day `2026-05-13`; local query/title packet at `2026-05-14 12:43` maps zero-click visible terms to paid-cohort candidates; public PDP preflight at `2026-05-14 13:00` narrows the authenticated export scope to `18/24` public-clean candidate rows; held-PDP repair packet at `2026-05-14 13:18` splits the remaining rows into `3` excluded until supplier-clean, `2` excluded until stale-copy-clean, and `1` weak-fit row allowed only if item-level impressions prove relevance. No negative/title/product-group write until authenticated item-level export proof exists |
...
| GB/CA/AU candidate collection routes | n/a | n/a | n/a | n/a | n/a | n/a | `36_ROW_CPC_VALIDATION_PACKET_READY__SWIM_REROUTED_CLEAN` | Public source preflight/reroute, `2026-05-14 12:19 EDT`: `mommy-and-me`, `family-matching`, `pajamas`, and `family-swimsuits` clean across GB/CA/AU with `200` and `0` supplier/url-brand hits; exact packet selects `36` rows (`GB=12`, `CA=12`, `AU=12`); `/collections/swimsuits` remains excluded |
